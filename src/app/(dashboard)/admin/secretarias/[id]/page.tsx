...
// ==============================
// TIPOS PARA SECRETARIA
// ==============================

interface SecretariaEstadisticas {
...
interface SecretariaDetalle {
  ...
  estadisticas: SecretariaEstadisticas;

  ultimas_actividades: SecretariaActividad[];
}
...
export default function DetalleSecretariaPage() {
  const router = useRouter();
  const params = useParams() as { id: string };

  const [secretaria, setSecretaria] = useState<SecretariaDetalle | null>(null);
  ...
  // ====== DATA LOAD ======
  const cargarSecretaria = async () => {
    try {
      setLoading(true);
      setError("");
      const res = await fetch(`/api/admin/secretarias/${params.id}`);
      const data = await res.json();
      if (data.success) {
        setSecretaria(data.data as SecretariaDetalle);
      } else {
        setError(data.error || "Error al cargar secretaria");
      }
    } catch (e) {
      console.error(e);
      setError("Error al cargar secretaria");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEliminar = async () => {
    try {
      setDeleting(true);
      const response = await fetch(`/api/admin/secretarias/${params.id}`, {
        method: "DELETE",
      });
      const data = await response.json();
      if (data.success) {
        router.push("/admin/secretarias?deleted=true");
      } else {
        setErr(data.error || "Error al eliminar secretaria");
        setShowDeleteModal(false);
      }
    } catch {
      setErr("Error al eliminar secretaria");
      setShowDeleteModal(false);
    } finally {
      setDeleting(false);
    }
  ...
  const enable2FAInit = async () => {
    try {
      if (!secretaria) {
        setErr("No se encontró la secretaria");
        return;
      }
  ...
  const lockUser = async () => {
    try {
      ...
      setSecLoading("lock");
      const data = await post(
        `/api/admin/usuarios/${userId}/security/lock`
      );
      data.success
  ...
  const unlockUser = async () => {
    try {
      ...
      const data = await post(
        `/api/admin/usuarios/${userId}/security/unlock`
      );
      data.success
  ...
  const exportUser = async () => {
    try {
      ...
      await downloadFromResponse(res, `secretaria_${userId}.json`);
  ...
  const exportLogs = async () => {
    try {
      ...
      if (!res.ok) throw new Error();
      await downloadFromResponse(res, `secretaria_${userId}_logs.json`);
        ...
        <div className="text-center relative">
          ...
          <div className="relative z-10">
            ...
            >
              ...
              Cargando perfil de secretaria
            </h3>
            <p className={`${textSecondary} font-medium animate-pulse`}>
              Preparando datos de la secretaria...
            </p>
          </div>
  ...
  const stats = secretaria.estadisticas || ({} as SecretariaEstadisticas);
    ...
    >
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative z-10">
          ...
          <div className="p-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                ...
                <div className="relative group">
                  {secretaria.foto_perfil_url ? (
                    <>
                      ...
                      ></div>
                      <img
                        src={secretaria.foto_perfil_url}
                        alt={secretaria.nombre_completo}
                    ...
                    <>
                      ...
                      >
                        <span className="text-3xl font-black text-white">
                          {secretaria.nombre?.charAt(0)}
                          {secretaria.apellido_paterno?.charAt(0)}
                        </span>
                      </div>
                    </>
                  )}

                  {secretaria.autenticacion_doble_factor && (
                    <div
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h1 className={`text-3xl font-black ${textPrimary}`}>
                      {secretaria.nombre_completo}
                    </h1>
                    {getEstadoBadge(secretaria.estado)}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm mb-3">
                    ...
                    >
                      ...
                      <span>{secretaria.rut}</span>
                    </div>
                  </div>

                  {secretaria.roles_nombres && (
              ...
              <div className="flex flex-wrap gap-3">
                <Link
                  href={`/admin/secretarias/${params.id}/editar`}
        ...
        >
          ...
          {activeTab === "general" && (
            <div className="space-y-6 animate-in fade-in zoom-in duration-300">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Datos básicos */}
                {[
                  { label: "RUT", value: secretaria.rut, icon: FileText },
                  { label: "Email", value: secretaria.email, icon: Mail },
                  {
                    label: "Teléfono",
                    value: secretaria.telefono || "No registrado",
                    icon: Phone,
                  },
                  {
                    label: "Celular",
                    value: secretaria.celular || "No registrado",
                    icon: Phone,
                  },
                  {
                    ...
                    value: secretaria.fecha_nacimiento
                      ? `${formatFechaCorta(
                          secretaria.fecha_nacimiento
                        )}${secretaria.edad ? ` (${secretaria.edad} años)` : ""}`
                      : "No registrada",
                    icon: Calendar,
                  },
                  {
                    label: "Género",
                    value:
                      secretaria.genero?.replace("_", " ") || "No especificado",
                    icon: User,
                  },
                ].map((field, index) => {
                ...
                >
                  ...
                  <p className={`text-lg font-semibold ${textPrimary}`}>
                    {secretaria.direccion || "No registrada"}
                  </p>
                </div>
                ...
                {[
                  {
                    label: "Ciudad",
                    value: secretaria.ciudad || "No registrada",
                    icon: MapPin,
                  },
                  {
                    label: "Región",
                    value: secretaria.region || "No registrada",
                    icon: MapPin,
                  },
                  {
                    label: "Fecha de Creación",
                    value: formatFecha(secretaria.fecha_creacion),
                    icon: Calendar,
                  },
                  {
                    label: "Última Modificación",
                    value: formatFecha(secretaria.fecha_modificacion),
                    icon: Clock,
                  },
                ].map((field, index) => {
                  const Icon = field.icon as any;
                ...
                {[
                  {
                    label: "Jornada",
                    value: formatJornada(secretaria.jornada),
                    icon: Clock,
                  },
                  {
                    label: "Extensión Telefónica",
                    value:
                      secretaria.extension_telefonica || "No registrada",
                    icon: Phone,
                  },
                ].map((field, index) => {
                  const Icon = field.icon as any;
          ...
          {activeTab === "roles" && (
            <div className="space-y-6 animate-in fade-in zoom-in duration-300">
              {secretaria.roles_detallados &&
              secretaria.roles_detallados.length > 0 ? (
                secretaria.roles_detallados.map((rol, index) => (
                  <div
                    key={rol.id_rol}
          ...
          {activeTab === "actividad" && (
            <div className="space-y-4 animate-in fade-in zoom-in duration-300">
              {secretaria.ultimas_actividades &&
              secretaria.ultimas_actividades.length > 0 ? (
                secretaria.ultimas_actividades.map((log, idx) => (
                  <div
                    key={idx}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <div className={`${textSecondary} font-medium`}>
                    ...
                    <span
                      className={`font-bold ${
                        secretaria.requiere_cambio_password
                          ? "text-amber-600 dark:text-amber-400"
                          : "text-slate-600 dark:text-slate-400"
                      }`}
                    >
                      {secretaria.requiere_cambio_password ? "Sí" : "No"}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    <button
