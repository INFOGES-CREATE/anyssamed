// frontend/src/app/(dashboard)/admin/secretarias/nueva/page.tsx
"use client";
...
export default function NuevaSecretariaPage() {
  ...
  const [medicosFiltrados, setMedicosFiltrados] = useState<MedicoOption[]>([]);
  const [supervisoresFiltrados, setSupervisoresFiltrados] = useState<SupervisorOption[]>([]);
  ...
  // 🔍 Usuario buscado por RUT
  const [usuarioExistente, setUsuarioExistente] = useState<UsuarioBasico | null>(null);
  const [buscandoUsuario, setBuscandoUsuario] = useState(false);
  const [mensajeUsuarioRut, setMensajeUsuarioRut] = useState<string | null>(null);
  ...
  const cargarCatalogos = async () => {
    try {
      ...
      console.log("🌍 Cargando catálogos para secretarias...");
      ...
      try {
        ...
        if (dataDept.success && Array.isArray(dataDept.data)) {
          const activos = dataDept.data.filter(
            (d: Departamento) => d.estado === "activo"
          );
          setDepartamentos(activos);
          console.log("✅ Departamentos cargados:", activos.length);
        }
      } catch (error) {
        console.error("❌ Error al cargar departamentos:", error);
      }

      // 6️⃣ Médicos (para asignación)
      try {
        const resMedicos = await fetch("/api/medicos/opciones");
        const dataMedicos = await resMedicos.json();
        if (dataMedicos.success && Array.isArray(dataMedicos.data)) {
          const activos = dataMedicos.data.filter(
            (m: MedicoOption) => m.estado === "activo"
          );
          setMedicos(activos);
          console.log("✅ Médicos cargados:", activos.length);
        }
      } catch (error) {
        console.error("❌ Error al cargar médicos:", error);
      }

      // 7️⃣ Supervisores (administrativos jefes)
      try {
        const resSup = await fetch("/api/administrativos/opciones");
        const dataSup = await resSup.json();
        if (dataSup.success && Array.isArray(dataSup.data)) {
          const activos = dataSup.data.filter(
            (s: SupervisorOption) => s.estado === "activo"
          );
          setSupervisores(activos);
          console.log("✅ Supervisores cargados:", activos.length);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (formData.id_pais) {
      const centrosFiltro = centros.filter((c) => c.estado === "activo");
  ...
  // ==============================
  // 🏢 SUCURSALES POR CENTRO + FILTRO DEPT/MÉDICOS/SUPERVISORES
  // ==============================
  useEffect(() => {
    if (formData.id_centro_principal) {
      const centroId = parseInt(formData.id_centro_principal);

      const sucursalesFiltro = sucursales.filter(
        (s) => s.id_centro === centroId && s.estado === "activo"
      );
      ...
      const deptFiltro = departamentos.filter(
        (d) =>
          d.estado === "activo" &&
          (!d.id_centro || d.id_centro === centroId)
      );
      setDepartamentosFiltrados(deptFiltro);

      const medicosFiltro = medicos.filter(
        (m) =>
          m.estado === "activo" &&
          (!m.id_centro || m.id_centro === centroId)
      );
      setMedicosFiltrados(medicosFiltro);

      const supervisoresFiltro = supervisores.filter(
        (s) =>
          s.estado === "activo" &&
          (!s.id_centro || s.id_centro === centroId)
      );
      setSupervisoresFiltrados(supervisoresFiltro);
    } else {
      ...
      setMedicosFiltrados(
        medicos.filter((m) => m.estado === "activo")
      );
      setSupervisoresFiltrados(
        supervisores.filter((s) => s.estado === "activo")
      );
    }
  }, [
  ...
  // Buscar usuario por RUT cuando sea válido
  useEffect(() => {
    if (!formData.rut) {
      setUsuarioExistente(null);
      setMensajeUsuarioRut(null);
      return;
    }
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        setMensajeUsuarioRut("Buscando usuario por RUT...");

        const res = await fetch(
          `/api/users/buscar-por-rut?rut=${encodeURIComponent(formData.rut)}`,
          {
            method: "GET",
            credentials: "include",
        ...
        if (!res.ok) {
          console.error("Error HTTP al buscar usuario por RUT:", res.status);
          setUsuarioExistente(null);
          setMensajeUsuarioRut("No se pudo verificar el usuario por RUT.");
          return;
        }
        ...
        if (data.success && data.usuario) {
          const u: UsuarioBasico = data.usuario;
          setUsuarioExistente(u);
          setMensajeUsuarioRut(
            "✅ Usuario encontrado. Se rellenaron los datos automáticamente."
          );

          setFormData((prev) => ({
            ...prev,
            username: u.nombre_usuario || prev.username,
            email: (u.email ?? u.correo ?? prev.email) || prev.email,
            nombre: u.nombres || prev.nombre,
            apellido_paterno: u.apellidos || prev.apellido_paterno,
            foto_perfil_url: u.foto_perfil || prev.foto_perfil_url,
          }));
          ...
          setValidaciones((prev) => ({
            ...
            username: {
              valido: true,
              mensaje: "Username cargado desde usuario existente",
            },
            email: {
              valido: true,
              mensaje: "Email cargado desde usuario existente",
            },
            password: {
              valido: true,
              mensaje: "Se usará la contraseña actual del usuario",
            },
            confirmPassword: {
              valido: true,
              mensaje:
                "No es necesario confirmar contraseña para usuario existente",
            },
          }));
        } else {
          setUsuarioExistente(null);
          setMensajeUsuarioRut(
            "No existe usuario con ese RUT. Se creará un usuario nuevo al guardar."
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!usuarioExistente && !validaciones.password.valido) {
      ...
      if (formData.roles.length === 0) {
        setError("Debe seleccionar al menos un rol (incluyendo el de secretaria)");
        setLoading(false);
        return;
      }
      ...
      if (!formData.id_centro_principal) {
        setError("Debe seleccionar un centro médico principal");
        setLoading(false);
        return;
      }

      if (!formData.fecha_inicio_secretaria) {
        setError("Debe indicar la fecha de inicio de la secretaria");
        setLoading(false);
        return;
      }

      const bodyToSend = {
        // Si ya existe usuario en tabla usuarios, lo indicamos al backend
        id_usuario_existente: usuarioExistente ? usuarioExistente.id_usuario : null,
        ...
        autenticacion_doble_factor: formData.autenticacion_doble_factor,
        enviar_email_bienvenida: formData.enviar_email_bienvenida,

        // SECRETARIA
        estado_secretaria: formData.estado_secretaria,
        jornada: formData.jornada,
        extension_telefonica_secretaria:
          formData.extension_telefonica_secretaria || null,
        fecha_inicio_secretaria: formData.fecha_inicio_secretaria,
        fecha_termino_secretaria: formData.fecha_termino_secretaria || null,
        ...
        supervisor_id: formData.supervisor_id
          ...
          : null,
        numero_empleado: formData.numero_empleado || null,

        // Asignación a médico principal -> medicos_asignados
        medicos_asignados: formData.id_profesional_principal
          ? [
              {
                id_profesional: parseInt(formData.id_profesional_principal),
                es_principal: true,
      ...
      const response = await fetch("/api/admin/secretarias/nueva", {
        method: "POST",
      ...
      if (data.success) {
        ...
        setError(data.error || "Error al crear secretaria");
      }
    } catch (error) {
      console.error("❌ Error:", error);
      setError("Error al crear secretaria");
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="text-center relative z-10">
          ...
          <h3 className={`text-2xl font-black ${textPrimary} mb-2`}>
            ...
            Cargando Formulario de Secretaria
          </h3>
          <p className={`${textSecondary} font-medium`}>
            Obteniendo datos de países, centros, departamentos y roles...
          </p>
        </div>
      </div>
    );
    ...
    >
      ...
      <div className="max-w-6xl mx-auto space-y-6 relative z-10">
        ...
        >
          ...
          <div className="p-6">
            <div className="flex items-center gap-4">
              <button
                onClick={() => router.push("/admin/secretarias")}
        ...
        {success && (
          ...
          >
            ...
            <div className="flex-1">
              ...
              >
                Redirigiendo al panel de secretarias...
              </p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      placeholder="secretaria@centro.cl"
                    />
                    <div className="absolute right-4 top-1/2 transform -translate-y-1/2">
                ...
                <div>
                  <label
                    className={`block text-sm font-black mb-2 uppercase tracking-wider ${textMuted}`}
                  >
                    Contraseña {usuarioExistente ? "(opcional, usuario existente)" : "*"}
                  </label>
                  <div className="relative group">
                    <Lock
                      className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 ${textMuted}`}
                    ...
                    <input
                      ...
                      placeholder={usuarioExistente ? "•••••••• (se mantiene actual)" : "••••••••"}
                    />
                ...
                <div>
                  <label
                    className={`block text-sm font-black mb-2 uppercase tracking-wider ${textMuted}`}
                  >
                    Confirmar Contraseña {usuarioExistente ? "(no requerido si ya existe)" : "*"}
                  </label>
                  <div className="relative group">
                    <Lock
                      className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 ${textMuted}`}
              ...
              >
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  >
                    Enviar email de bienvenida a la secretaria
                  </span>
                </label>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center gap-2">
                      ...
                      {usuarioExistente && !buscandoUsuario && (
                  ...
                  {mensajeUsuarioRut && (
                    <p
                      className={`text-xs mt-1 font-semibold ${
                        usuarioExistente ? "text-emerald-400" : "text-amber-400"
                      }`}
                    >
                      {mensajeUsuarioRut}
                    </p>
          ...
          {/* DATOS LABORALES SECRETARIA */}
          <div
            className={`${cardBg} rounded-3xl shadow-2xl border ${borderColor} p-6 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-hidden relative`}
            style={{ animationDelay: "450ms" }}
          >
            <div
              className={`absolute -top-1/2 -right-1/2 w-full h-full bg-gradient-to-br from-amber-500 to-orange-600 opacity-5 blur-3xl`}
            ></div>
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className={`text-2xl font-black ${textPrimary}`}>
                  Datos Laborales de la Secretaria
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select
                    name="estado_secretaria"
                    value={formData.estado_secretaria}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 rounded-xl transition-all duration-300 font-medium border-2 cursor-pointer ${
                      darkMode
                        ? "bg-slate-900/50 border-white/10 text-white focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/20"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="extension_telefonica_secretaria"
                    value={formData.extension_telefonica_secretaria}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 rounded-xl transition-all duration-300 font-medium border-2 ${
                      darkMode
                        ? "bg-slate-900/50 border-white/10 text-white placeholder-slate-500 focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/20"
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input
                      type="date"
                      name="fecha_inicio_secretaria"
                      value={formData.fecha_inicio_secretaria}
                      onChange={handleInputChange}
                      required
                      className={`w-full pl-12 pr-4 py-3 rounded-xl transition-all duration-300 font-medium border-2 ${
                        darkMode
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input
                      type="date"
                      name="fecha_termino_secretaria"
                      value={formData.fecha_termino_secretaria}
                      onChange={handleInputChange}
                      className={`w-full pl-12 pr-4 py-3 rounded-xl transition-all duration-300 font-medium border-2 ${
                        darkMode
                          ? "bg-slate-900/50 border-white/10 text-white placeholder-slate-500 focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/20"
                ...
                <div>
                  ...
                  >
                    ...
                    {supervisoresFiltrados.map((s) => (
                      <option
                        key={s.id_administrativo}
                        value={s.id_administrativo}
                      >
                        {s.nombre_completo}
                        {s.cargo ? ` - ${s.cargo}` : ""}
                      </option>
                ...
                <div>
                  <label
                    className={`block text-sm font-black mb-2 uppercase tracking-wider ${textMuted}`}
                  >
                    Médico Principal Asignado
                  </label>
                  <select
                    name="id_profesional_principal"
                    value={formData.id_profesional_principal}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 rounded-xl transition-all duration-300 font-medium border-2 cursor-pointer ${
                      darkMode
                        ? "bg-slate-900/50 border-white/10 text-white focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/20"
                  ...
                  >
                    <option value="">(Opcional) Seleccionar médico...</option>
              ...
              >
                ...
                >
                  <AlertCircle className="w-4 h-4" />
                  Estos datos se usan para indicadores, reportes y paneles de
                  RRHH de la secretaria.
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className={`text-sm ${textMuted} mt-1`}>
                    Asigna los roles adecuados para la secretaria *
                  </p>
                </div>
              </div>
                ...
                >
                  ...
                  >
                    {!formData.id_pais
                      ...
                      : "Debe crear roles en el sistema antes de crear secretarias"}
                  </p>
                </div>
              ) : (
                <>
                  ...
                  {formData.roles.length === 0 && (
                    ...
                    >
                      ...
                      >
                        ...
                        Debe seleccionar al menos un rol para la secretaria
                      </p>
                    </div>
                  )}
                </>
                  ...
                  <div className="mt-4 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <p className={`text-sm ${textMuted}`}>
                        La imagen se mostrará en el perfil de la secretaria en
                        todas las vistas.
                      </p>
                    </div>
                  </div>
          ...
          >
            ...
            <div className="relative z-10">
              <h2 className={`text-2xl font-black ${textPrimary} mb-6`}>
                📋 Resumen de Datos de la Secretaria
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <p className={`text-lg font-black ${textPrimary}`}>
                    {formData.estado_secretaria} / {formData.jornada}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div
                  className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-300 ${
                    (validaciones.password.valido || usuarioExistente)
                      ? darkMode
                        ? "bg-emerald-500/10 border border-emerald-500/30"
                        : "bg-emerald-50 border border-emerald-200"
                      : darkMode
                ...
                >
                  ...
                  >
                    Contraseña:{" "}
                    {usuarioExistente
                      ? "✓ Usuario existente (no requiere cambio)"
                      : validaciones.password.valido
                      ? "✓ Segura"
                      : "✗ Débil"}
                  </span>
                    ...
                    style={{
                      width: `${
                        (Object.values(validaciones).filter((v) => v.valido)
                          .length / Object.values(validaciones).length) *
                        100
                      }%`,
                    }}
                  ></div>
          ...
          >
            ...
            <button
              ...
              disabled={
                ...
                !validaciones.username.valido ||
                !validaciones.email.valido ||
                (!usuarioExistente && !validaciones.password.valido) ||
                !formData.id_pais ||
                !formData.id_centro_principal ||
                !formData.fecha_inicio_secretaria
              }
              className={`flex-1 px-6 py-4 rounded-2xl transition-all duration-300 font-black text-lg uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed shadow-2xl transform hover:-translate-y-1 ${
                darkMode
              ...
              <div className="flex items-center justify-center gap-2">
                {loading ? (
                  <>
                    ...
                    <span>Creando secretaria...</span>
                  </>
                ) : (
                  <>
                    <Save className="w-5 h-5" />
                    <span>Crear Secretaria</span>
                  </>
                )}
              </div>
            </button>
