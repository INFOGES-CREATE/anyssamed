// frontend/src/app/(dashboard)/admin/usuarios/[id]/estadisticas/page.tsx
"use client";
...
export default function EstadisticasUsuarioPage({
  ...
  const porcentajeExito = Number(estadisticas?.tasa_exito?.porcentaje_exito ?? 0);
  ...
  const exportarEstadisticas = (formato: "json" | "csv") => {
    ...
    if (formato === "json") {
      ...
      link.href = url;
      link.download = `estadisticas-usuario-${params.id}-${new Date().toISOString().split("T")[0]}.json`;
      link.click();
    } else if (formato === "csv") {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `estadisticas-usuario-${params.id}-${new Date().toISOString().split("T")[0]}.csv`;
      link.click();
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative z-10">
        ...
        >
          ...
          <StatCard
            ...
            trend={
              resumen.actividad_hoy > 0
                ? Math.round(
                    (resumen.actividad_hoy / (resumen.actividad_semana / 7)) *
                      100 -
                      100
                  )
                : 0
            ...
            <ResponsiveContainer width="100%" height={300}>
              <RechartsPieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={(entry) => `${entry.origen}: ${entry.cantidad}`}
                  outerRadius={90}
                  fill="#8884d8"
                  dataKey="cantidad"
                >
                    ...
                    data={[
                      {
                        name: "Pagadas",
                        value:
                          estadisticas.estadisticas_pagos.citas_pagadas,
                      },
                      {
                        name: "Sin Pagar",
                        value:
                          estadisticas.estadisticas_pagos.citas_sin_pagar,
                      },
                    ]}
                    cx="50%"
                    cy="50%"
                ...
                <div className="relative w-40 h-40">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle
                      ...
                      strokeWidth="8"
                      strokeDasharray={`${
                        (estadisticas.tasa_exito.porcentaje_exito / 100) *
                        440
                      } 440`}
                      strokeLinecap="round"
          ...
          >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                >
                  {estadisticas.auditoria_reciente.slice(0, 15).map((audit, idx) => (
                    <tr
                      key={idx}
                      className={`transition-colors duration-200 ${
                        darkMode ? "hover:bg-slate-700/50" : "hover:bg-slate-50"
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={estadisticas.distribucion_severidad}>
                ...
                <XAxis
                  ...
                  stroke={darkMode ? "#94a3b8" : "#64748b"}
                  style={{ fontSize: "12px", fontWeight: 600 }}
                  label={{ value: "Nivel", position: "insideBottomRight", offset: -5 }}
                />
                <YAxis
                  stroke={darkMode ? "#94a3b8" : "#64748b"}
                  style={{ fontSize: "12px", fontWeight: 600 }}
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className={`text-sm ${textMuted}`}>
{item.cantidad} eventos ({Number(item.porcentaje ?? 0).toFixed(1)}%)
                      </p>
                    </div>
                    <p
