// src/app/(dashboard)/secretaria/configuracion/integraciones/page.tsx
"use client";
...
// Integraciones del centro (desde tu BD / API)
interface IntegracionCentro {
  id_integracion: number;
  codigo: string; // ej: "whatsapp_business", "sms_gateway", "correo_institucional"
  nombre: string;
  descripcion: string;
  categoria: "mensajeria" | "agenda" | "reportes" | "telefonia" | "otros";
  proveedor: string | null;
  estado: "conectado" | "configuracion_requerida" | "no_configurado" | "proximamente";
...
export default function ConfiguracionSecretariaPage() {
  ...
  // Datos generales para badges del menú (vendrán de tu API luego)
  ...
  const [integraciones, setIntegraciones] = useState<IntegracionCentro[]>([]);
  ...
  // Pestaña por defecto: INTEGRACIONES (este archivo)
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          icono: PlugIcon,
          url: "/secretaria/configuracion/integraciones",
        },
        {
          titulo: "Accesibilidad",
          icono: Eye,
          url: "/secretaria/configuracion/accesibilidad",
        },
      ],
    },
  ];
  ...
  // Cargar integraciones cuando haya usuario y la pestaña activa sea "integraciones"
  useEffect(() => {
    if (usuario && tabActiva === "integraciones") {
      cargarIntegracionesCentro();
    }
  }, [usuario, tabActiva]);
  ...
  const cargarIntegracionesCentro = async () => {
    if (!usuario?.secretaria?.centro?.id_centro) return;

    try {
      setLoadingIntegraciones(true);
      const res = await fetch(
        `/api/centros/${usuario.secretaria.centro.id_centro}/integraciones`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (res.ok && data.success && Array.isArray(data.integraciones)) {
        setIntegraciones(data.integraciones as IntegracionCentro[]);
      } else {
        setIntegraciones([]);
      }
    } catch (error) {
      console.error("Error al cargar integraciones del centro:", error);
      setIntegraciones([]);
    } finally {
  ...
  const obtenerColorEstadoIntegracion = (estado: string) => {
    const isDark = ["dark", "blue", "purple", "green"].includes(temaActual);
    const base = (light: string, dark: string) => (isDark ? dark : light);
  ...
  const obtenerTextoEstadoIntegracion = (estado: string) => {
    switch (estado) {
      case "conectado":
        return "Conectado";
      case "configuracion_requerida":
  ...
  const obtenerIconoIntegracion = (codigo: string) => {
    const key = codigo.toLowerCase();
    if (key.includes("whatsapp")) return MessageSquare;
    if (key.includes("sms")) return Phone;
    if (key.includes("correo") || key.includes("email")) return Mail;
    if (key.includes("telefonia") || key.includes("ip")) return PhoneCall;
    if (key.includes("calendar")) return Calendar;
    if (key.includes("powerbi") || key.includes("bi")) return BarChart3;
    return PlugIcon;
  };
  ...
  const manejarGestionIntegracion = (integracion: IntegracionCentro) => {
    if (integracion.url_panel) {
      window.open(integracion.url_panel, "_blank", "noopener,noreferrer");
      return;
    }

    mostrarMensajeEstado(
      "La configuración detallada de esta integración se realiza desde el panel de administración del centro.",
      "info"
    );
  };
  ...
  const renderContenidoTab = () => {
    ...
    // En este archivo nos interesa sobre todo la pestaña "integraciones".
    // Pero dejamos el resto para mantener consistencia con las demás páginas.

    if (tabActiva === "integraciones") {
      const total = integraciones.length;
      const conectadas = integraciones.filter(
        (i) => i.estado === "conectado"
      ).length;
      const pendientes = integraciones.filter((i) =>
        ["configuracion_requerida", "no_configurado"].includes(i.estado)
      ).length;
      const proximas = integraciones.filter(
        (i) => i.estado === "proximamente"
      ).length;

      return (
        <div className="space-y-6">
          {/* Resumen superior de integraciones */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <p className={`text-xs ${tema.colores.textoSecundario}`}>
                Plataformas conectadas en tiempo real con tu centro (mensajería, correo,
                telefonía, agenda externa, BI, etc.).
              </p>
            </div>
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-sm font-bold ${tema.colores.texto}`}>
                      Conectadas: {conectadas} · Pendientes: {pendientes}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 mt-2">
                <div className="flex items-center justify-between text-[11px]">
                  <span className={tema.colores.textoSecundario}>Conectadas</span>
                  <span className={tema.colores.texto}>
                    {total > 0
                      ? `${Math.round((conectadas / total) * 100)}%`
                      : "0%"}
                  </span>
                </div>
                <div className="w-full h-2 rounded-full bg-slate-600/30 overflow-hidden">
                  <div
                    className="h-2 rounded-full bg-gradient-to-r from-emerald-400 via-cyan-400 to-sky-400 transition-all duration-500"
                    style={{
                      width: `${total > 0 ? (conectadas / total) * 100 : 0}%`,
                    }}
                  />
                </div>
                <p className={`text-[11px] mt-1 ${tema.colores.textoSecundario}`}>
                  Mientras más integraciones activas tengas, menos trabajo manual
                  realizará la secretaria.
                </p>
              </div>
            </div>
            ...
            <div
              className={`rounded-2xl p-5 ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.sombra}`}
            >
              <p className={`text-xs font-semibold mb-2 ${tema.colores.textoSecundario}`}>
                Próximas integraciones
              </p>
              <p className={`text-2xl font-black mb-1 ${tema.colores.texto}`}>
                {proximas}
              </p>
              <p className={`text-xs mb-4 ${tema.colores.textoSecundario}`}>
                Funcionalidades que el centro podrá activar en los próximos meses.
              </p>
              <button
                type="button"
                onClick={() =>
                  mostrarMensajeEstado(
                    "Para habilitar nuevas integraciones, coordina con el administrador comunal o soporte AnyssaMed.",
                    "info"
                  )
                }
                className={`w-full flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-xs font-bold border ${tema.colores.card} ${tema.colores.borde} ${tema.colores.texto} hover:${tema.colores.hover}`}
              >
                <Rocket className="w-4 h-4" />
                Ver opciones de expansión
              </button>
            </div>
          </div>

          {/* Bloque principal de integraciones */}
          <div
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-xs ${tema.colores.textoSecundario}`}>
                    Conecta AnyssaMed con WhatsApp, SMS, correo institucional, telefonía
                    IP, calendarios externos y herramientas de BI.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className={`px-3 py-1 rounded-full text-[11px] font-bold bg-gradient-to-r ${tema.colores.gradiente} text-white`}
                >
                ...
                <span className={`text-[11px] ${tema.colores.textoSecundario}`}>
                  ...
                  integraciones.some((i) => i.ultima_actualizacion)
                    ? formatearFecha(
                        integraciones
                          .map((i) => i.ultima_actualizacion)
                          .filter(Boolean)
                          .sort()
                          .slice(-1)[0] as string
                      )
            ...
            {loadingIntegraciones ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mt-2">
                {Array.from({ length: 3 }).map((_, idx) => (
                  <div
                    key={idx}
                    className={`rounded-2xl p-4 ${tema.colores.card} ${tema.colores.borde} border animate-pulse`}
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-9 h-9 rounded-xl bg-slate-500/20" />
                      <div className="flex-1 space-y-2">
                        <div className="h-3 rounded-full bg-slate-500/30 w-1/2" />
                        <div className="h-2 rounded-full bg-slate-500/20 w-2/3" />
                      </div>
                    </div>
                    <div className="h-2 rounded-full bg-slate-500/20 mb-2" />
                    <div className="h-2 rounded-full bg-slate-500/10 w-3/4" />
                  </div>
                ))}
              </div>
            ) : integraciones.length === 0 ? (
              <div className="py-10 text-center">
                <div className="w-16 h-16 rounded-2xl bg-slate-500/10 flex items-center justify-center mx-auto mb-4">
                  <PlugIcon className="w-8 h-8 text-slate-400" />
                </div>
                <p className={`text-sm font-semibold mb-1 ${tema.colores.texto}`}>
                  Aún no hay integraciones configuradas para este centro.
                </p>
                <p className={`text-xs mb-4 ${tema.colores.textoSecundario}`}>
                  El administrador comunal o el equipo de soporte pueden activar
                  integraciones como WhatsApp, SMS, correo institucional o BI.
                </p>
                <button
                  type="button"
                  onClick={() =>
                    mostrarMensajeEstado(
                      "Contacta a soporte o al administrador comunal para habilitar integraciones.",
                      "info"
                    )
                  }
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold border ${tema.colores.card} ${tema.colores.borde} ${tema.colores.texto} hover:${tema.colores.hover}`}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mt-2">
                {integraciones.map((int) => {
                  const Icono = obtenerIconoIntegracion(int.codigo);
                  const badgeColor = obtenerColorEstadoIntegracion(int.estado);

                  return (
                    <div
                      key={int.id_integracion}
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl bg-slate-900/10 flex items-center justify-center">
                              <Icono className="w-5 h-5 text-indigo-400" />
                            </div>
                            <div>
                              <p
                              ...
                              <p
                                className={`text-[11px] uppercase tracking-[0.15em] ${tema.colores.textoSecundario}`}
                              >
                                {int.categoria === "mensajeria" && "MENSAJERÍA"}
                                {int.categoria === "agenda" && "AGENDAS"}
                                {int.categoria === "reportes" && "REPORTES / BI"}
                                {int.categoria === "telefonia" && "TELEFONÍA IP"}
                                {int.categoria === "otros" && "OTROS"}
                                {int.proveedor ? ` · ${int.proveedor}` : ""}
                              </p>
                            </div>
                          </div>
                          <span
                            className={`px-2 py-1 rounded-full text-[10px] font-bold border ${badgeColor}`}
                          >
                            {obtenerTextoEstadoIntegracion(int.estado)}
                          </span>
                        </div>

                        <p
                          className={`text-[11px] mb-3 ${tema.colores.textoSecundario}`}
                        >
                          {int.descripcion}
                        </p>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          {int.permite_envio && (
                            <span className="px-2 py-1 rounded-full text-[10px] font-semibold bg-emerald-500/10 text-emerald-300 border border-emerald-500/30">
                              Permite envíos desde AnyssaMed
                            </span>
                          )}
                          {int.requiere_credenciales && (
                            <span className="px-2 py-1 rounded-full text-[10px] font-semibold bg-amber-500/10 text-amber-300 border border-amber-500/30">
                      ...
                      <div className="mt-3 flex items-center justify-between text-[11px]">
                        <div className={tema.colores.textoSecundario}>
                          {int.ultima_actualizacion ? (
                            <>
                              Última actualización:{" "}
                              <span className={tema.colores.texto}>
                                {formatearFecha(int.ultima_actualizacion)}
                              </span>
                            </>
                          ) : (
                        ...
                        <button
                          type="button"
                          onClick={() => manejarGestionIntegracion(int)}
                          className="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-[11px] font-bold border border-indigo-500/40 text-indigo-300 hover:bg-indigo-500/10"
                        >
          ...
          {/* Nota final / ayuda rápida */}
          <div
            className={`rounded-2xl p-4 ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.sombra} flex flex-wrap items-center justify-between gap-4`}
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-9 h-9 bg-gradient-to-br ${tema.colores.gradiente} rounded-xl flex items-center justify-center`}
              >
                <InfoIcon />
              </div>
              <div>
                <p className={`text-sm font-bold ${tema.colores.texto}`}>
                  ¿Cómo se configuran las integraciones?
                </p>
                <p className={`text-xs ${tema.colores.textoSecundario}`}>
                  La mayoría de las integraciones se habilitan a nivel comunal. Una vez
                  activadas, aparecerán aquí y podrás ver su estado desde el panel de
                  secretaria.
                </p>
              </div>
            </div>
            <Link
              ...
              className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl text-[11px] font-bold border ${tema.colores.card} ${tema.colores.borde} ${tema.colores.texto} hover:${tema.colores.hover}`}
            >
    ...
    // Resto de pestañas (se dejan para consistencia con las otras páginas)
    // Puedes pegar aquí el mismo contenido que ya usas en configuracion/notificaciones, seguridad, temas, etc.
    // Para no hacer el archivo infinito, las dejo vacías pero listas.

    if (tabActiva === "general") {
      return (
        <div className="space-y-6">
          <div
            className={`rounded-2xl p-6 ${tema.colores.card} ${tema.colores.borde} border`}
          >
            <p className={`text-sm ${tema.colores.textoSecundario}`}>
              Desde esta página estás en la sección <b>Integraciones</b>. La pestaña
              general está disponible en{" "}
              <span className="font-semibold">/secretaria/configuracion</span>.
            </p>
          </div>
        </div>
      );
    ...
    if (tabActiva === "notificaciones") {
      return (
        <div className="space-y-6">
          ...
          >
            <p className={`text-sm ${tema.colores.textoSecundario}`}>
              Configura notificaciones detalladas en{" "}
              <span className="font-semibold">
                /secretaria/configuracion/notificaciones
              </span>
              .
            </p>
          </div>
    ...
    if (tabActiva === "seguridad") {
      return (
        <div className="space-y-6">
          ...
          >
            <p className={`text-sm ${tema.colores.textoSecundario}`}>
              La seguridad completa de cuenta está en{" "}
              <span className="font-semibold">
                /secretaria/configuracion/seguridad
              </span>
              .
            </p>
          </div>
    ...
    if (tabActiva === "temas") {
      return (
        <div className="space-y-6">
          ...
          >
            <p className={`text-sm ${tema.colores.textoSecundario}`}>
              Personaliza colores y apariencia avanzada en{" "}
              <span className="font-semibold">/secretaria/configuracion/temas</span>.
            </p>
          </div>
        </div>
      );
    ...
    if (tabActiva === "accesibilidad") {
      return (
        <div className="space-y-6">
          ...
          >
            <p className={`text-sm ${tema.colores.textoSecundario}`}>
              Ajustes de accesibilidad visual están disponibles en{" "}
              <span className="font-semibold">
                /secretaria/configuracion/accesibilidad
              </span>
              .
            </p>
          </div>
        ...
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex-1 max-w-2xl">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar integración, ayuda, atajos..."
                value={busqueda}
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              >
                Configuración · Integraciones del Centro
              </p>
              <h2
              ...
              >
                Administra cómo AnyssaMed se conecta con WhatsApp, SMS, correo, telefonía
                IP y otras plataformas externas del establecimiento.
              </p>
            </div>
            ...
            <div className="flex flex-col items-end gap-3">
              <div className="flex items-center gap-2 text-xs">
                ...
                >
                  Centro: {usuario.secretaria.centro.nombre}
                </span>
                <span className={`text-[11px] ${tema.colores.textoSecundario}`}>
