...
// Config para eventos de notificación por tipo de evento (dinámico desde BD)
type CanalBasico = "email" | "push" | "sms" | "whatsapp";

interface ConfigEventoNotificacion {
...
// Horarios de silencio (quiet hours) para canales
interface HorarioSilencioNotificacion {
  id_silencio: number;
  ...
  aplica_push: boolean;
  aplica_sms: boolean;
  aplica_whatsapp: boolean;
  activo: boolean;
}

// Config de resúmenes automáticos
interface ConfigResumenNotificaciones {
  resumen_diario_activado: boolean;
  resumen_diario_hora: string; // "HH:MM"
  resumen_semanal_activado: boolean;
  resumen_semanal_dia: number; // 1-7
  resumen_semanal_hora: string; // "HH:MM"
}
...
export default function ConfiguracionNotificacionesSecretariaPage() {
  // Usuario y sesión
  const [usuario, setUsuario] = useState<UsuarioSesion | null>(null);
  const [loading, setLoading] = useState(true);

  // Datos generales para badges del menú (dinámico desde API)
  const [estadisticas, setEstadisticas] = useState<EstadisticasSecretaria | null>(
    null
  );
  ...
  // Notificaciones recientes del panel (dinámicas)
  const [notificaciones, setNotificaciones] = useState<NotificacionSecretaria[]>(
    []
  );

  // Config avanzada de notificaciones desde BD
  const [eventosConfig, setEventosConfig] = useState<ConfigEventoNotificacion[]>(
    []
  );
  const [horariosSilencio, setHorariosSilencio] = useState<
    HorarioSilencioNotificacion[]
  >([]);
  const [resumenNotificaciones, setResumenNotificaciones] =
    useState<ConfigResumenNotificaciones | null>(null);

  const [loadingConfigNotificaciones, setLoadingConfigNotificaciones] =
    useState(true);
  ...
  useEffect(() => {
    ...
    const cargarPreferenciaTema = async () => {
      try {
        ...
        if (data.success && data.tema_color && TEMAS[data.tema_color as TemaColor]) {
  ...
  const cargarEstadisticas = async () => {
    try {
      const res = await fetch("/api/secretaria/dashboard/estadisticas", {
        method: "GET",
        credentials: "include",
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok && data.success && data.estadisticas) {
        setEstadisticas(data.estadisticas as EstadisticasSecretaria);
      }
    } catch (error) {
      console.error("Error al cargar estadísticas secretaria:", error);
    }
  };

  const cargarNotificacionesPanel = async () => {
    try {
      const res = await fetch("/api/secretaria/notificaciones?limit=20", {
        method: "GET",
        credentials: "include",
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok && data.success && Array.isArray(data.notificaciones)) {
        setNotificaciones(data.notificaciones as NotificacionSecretaria[]);
      }
    } catch (error) {
      console.error("Error al cargar notificaciones de panel:", error);
    }
  };

  const cargarConfigNotificaciones = async () => {
    try {
      setLoadingConfigNotificaciones(true);
      const res = await fetch("/api/secretaria/config/notificaciones", {
        method: "GET",
        credentials: "include",
      });
      const data = await res.json().catch(() => ({}));
      ...
      if (res.ok && data.success) {
        if (Array.isArray(data.eventos)) {
          setEventosConfig(data.eventos as ConfigEventoNotificacion[]);
        } else {
          setEventosConfig([]);
        }

        if (Array.isArray(data.horarios_silencio)) {
          setHorariosSilencio(data.horarios_silencio as HorarioSilencioNotificacion[]);
        } else {
          setHorariosSilencio([]);
        }
        ...
        if (data.resumenes) {
          setResumenNotificaciones(
            data.resumenes as ConfigResumenNotificaciones
          );
        } else {
          setResumenNotificaciones({
            resumen_diario_activado: false,
            resumen_diario_hora: "18:00",
            resumen_semanal_activado: false,
            resumen_semanal_dia: 1,
            resumen_semanal_hora: "18:30",
          });
        }
      } else {
        setEventosConfig([]);
      ...
      setLoadingConfigNotificaciones(false);
    }
  };
  ...
  const handleTogglePreferencia = (campo: CampoToggle) => {
  ...
  // Toggles mini para matrices
  const renderToggleMini = (activo: boolean | undefined) => (
    <div
      className={`relative inline-flex items-center h-6 w-10 rounded-full transition-colors ${
        activo ? "bg-emerald-500" : "bg-gray-500/60"
      }`}
    >
      <span
  ...
  // Días semana
  const nombreDiaSemana = (dia: number) => {
    const dias = [
      "Domingo",
      "Lunes",
      "Martes",
  ...
  // ========================================
  // HANDLERS CONFIG NOTIFICACIONES (BD)
  // ========================================

  const handleToggleCanalEvento = async (
    id_tipo_evento: number,
    canal: CanalBasico
  ) => {
    const eventoActual = eventosConfig.find(
      (e) => e.id_tipo_evento === id_tipo_evento
    );
    if (!eventoActual) return;

    const nuevoValor = !eventoActual.canales[canal];

    setEventosConfig((prev) =>
      prev.map((e) =>
        e.id_tipo_evento === id_tipo_evento
          ? {
              ...e,
              canales: {
                ...e.canales,
    ...
    try {
      const res = await fetch(
        "/api/secretaria/config/notificaciones/eventos/canal",
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!res.ok || !data.success) {
        ...
        mostrarMensajeEstado("Configuración actualizada.", "ok");
      }
    } catch (error) {
      console.error("Error al actualizar evento de notificación:", error);
      mostrarMensajeEstado(
        "Error al guardar cambios del evento. Verifica tu conexión.",
        "error"
      );
    }
  };

  const handleCambioAnticipacionEvento = async (
    id_tipo_evento: number,
    minutos: number
  ) => {
    setEventosConfig((prev) =>
      prev.map((e) =>
        e.id_tipo_evento === id_tipo_evento
          ? { ...e, minutos_anticipacion: minutos }
          : e
      )
    );
    ...
    try {
      const res = await fetch(
        "/api/secretaria/config/notificaciones/eventos/anticipacion",
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!res.ok || !data.success) {
        mostrarMensajeEstado(
          "No se pudo guardar la anticipación del evento.",
          "error"
        );
      } else {
        mostrarMensajeEstado("Anticipación actualizada.", "ok");
      }
    } catch (error) {
      console.error("Error al actualizar anticipación:", error);
      mostrarMensajeEstado(
        "Error al guardar la anticipación. Verifica tu conexión.",
        "error"
      );
    }
  };

  const handleToggleCanalSilencio = async (
    id_silencio: number,
    canal: CanalBasico
  ) => {
    const silencioActual = horariosSilencio.find(
      (h) => h.id_silencio === id_silencio
    );
    if (!silencioActual) return;
    ...
    const nuevoValor = !silencioActual[campo as keyof HorarioSilencioNotificacion];

    setHorariosSilencio((prev) =>
      prev.map((h) =>
        h.id_silencio === id_silencio
          ? {
              ...h,
              [campo]: nuevoValor,
            }
    ...
    try {
      const res = await fetch(
        "/api/secretaria/config/notificaciones/horarios-silencio/canal",
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!res.ok || !data.success) {
        mostrarMensajeEstado(
          "No se pudo guardar el horario de silencio.",
          "error"
        );
      } else {
        mostrarMensajeEstado("Horario de silencio actualizado.", "ok");
      }
    } catch (error) {
      console.error("Error al actualizar horario de silencio:", error);
      mostrarMensajeEstado(
        "Error al guardar el horario. Verifica tu conexión.",
        "error"
      );
    }
  };

  const handleToggleHorarioActivo = async (id_silencio: number) => {
    const silencioActual = horariosSilencio.find(
      (h) => h.id_silencio === id_silencio
    );
    if (!silencioActual) return;

    const nuevoValor = !silencioActual.activo;

    setHorariosSilencio((prev) =>
      prev.map((h) =>
        h.id_silencio === id_silencio ? { ...h, activo: nuevoValor } : h
      )
    );

    try {
      const res = await fetch(
        "/api/secretaria/config/notificaciones/horarios-silencio/estado",
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const handleActualizarResumenNotificaciones = async (
    cambios: Partial<ConfigResumenNotificaciones>
  ) => {
    const nuevaConfig = {
    ...
    setResumenNotificaciones(nuevaConfig);

    try {
      const res = await fetch(
        "/api/secretaria/config/notificaciones/resumenes",
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!res.ok || !data.success) {
        mostrarMensajeEstado(
          "No se pudo actualizar la configuración de resúmenes.",
          "error"
        );
      } else {
        mostrarMensajeEstado("Resúmenes configurados correctamente.", "ok");
      }
    } catch (error) {
      console.error("Error al actualizar resúmenes:", error);
      mostrarMensajeEstado(
        "Error al guardar los resúmenes. Verifica tu conexión.",
        "error"
      );
    }
  };
  ...
  const totalCanalesPersonalesActivos = useMemo(() => {
    if (!preferencias) return 0;
    let count = 0;
    if (preferencias.notificaciones_email) count++;
    if (preferencias.notificaciones_push) count++;
    if (preferencias.notificaciones_sonido) count++;
    if (preferencias.notificaciones_sms) count++;
    if (preferencias.whatsapp_recordatorios) count++;
    return count;
  }, [preferencias]);

  const totalEventosConfigurados = eventosConfig.length;

  const totalEventosCriticos = useMemo(
    () => eventosConfig.filter((e) => e.critico).length,
    [eventosConfig]
  );

  const totalHorariosSilencioActivos = useMemo(
    () => horariosSilencio.filter((h) => h.activo).length,
    [horariosSilencio]
  );
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          >
            Preparando tu centro de alertas inteligente...
          </p>
        </div>
      </div>
    );
  ...
  const renderContenidoPrincipal = () => {
    if (!preferencias || loadingPreferencias || loadingConfigNotificaciones) {
      return (
        <div className="flex items-center justify-center py-16">
          <div className="text-center">
            ...
            >
              Cargando tu configuración avanzada de notificaciones...
            </p>
          </div>
        </div>
      );
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <div>
              <p className={`text-xs font-semibold ${tema.colores.textoSecundario}`}>
                Canales personales activos
              </p>
              <p className={`text-2xl font-black mt-1 ${tema.colores.texto}`}>
                {totalCanalesPersonalesActivos}
              </p>
              <p className={`text-[11px] mt-1 ${tema.colores.textoSecundario}`}>
                Email, navegador, sonido, SMS, WhatsApp
              </p>
            </div>
          ...
          >
            <div>
              ...
              <p className={`text-2xl font-black mt-1 ${tema.colores.texto}`}>
                {totalEventosConfigurados}
              </p>
              <p className={`text-[11px] mt-1 ${tema.colores.textoSecundario}`}>
                {totalEventosCriticos} marcados como críticos
              </p>
            </div>
            <div className="flex items-center gap-2">
          ...
          >
            <div>
              ...
              <p className={`text-2xl font-black mt-1 ${tema.colores.texto}`}>
                {totalHorariosSilencioActivos}
              </p>
              <p className={`text-[11px] mt-1 ${tema.colores.textoSecundario}`}>
                Tramos activos para no molestar fuera de horario
              </p>
            </div>
            <div className="flex items-center gap-2">
        ...
        {/* Canales personales + mensajería pacientes */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {/* Canales de notificación personales */}
          <div
          ...
          >
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div
                  onClick={() => handleTogglePreferencia("notificaciones_email")}
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div
                  onClick={() => handleTogglePreferencia("notificaciones_push")}
                  className="cursor-pointer"
                >
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div
                  onClick={() => handleTogglePreferencia("notificaciones_sonido")}
                  className="cursor-pointer"
                >
          ...
          >
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div
                  onClick={() => handleTogglePreferencia("notificaciones_sms")}
                  className="cursor-pointer"
                >
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className={`text-xs ${tema.colores.textoSecundario}`}>
                    Enviar recordatorios automáticos por WhatsApp (si el centro lo
                    tiene habilitado).
                  </p>
                </div>
                <div
                  onClick={() =>
                    handleTogglePreferencia("whatsapp_recordatorios")
                  }
                  className="cursor-pointer"
                >
              ...
              {/* Resumen al iniciar */}
              <div className="flex items-center justify-between gap-4 pt-2 border-t border-dashed border-gray-500/40 mt-2">
                <div>
                  <p className={`text-sm font-semibold ${tema.colores.texto}`}>
                    Resumen al iniciar sesión
                  </p>
                  <p className={`text-xs ${tema.colores.textoSecundario}`}>
                ...
                <div
                  onClick={() =>
                    handleTogglePreferencia("notificacion_login")
                  }
                  className="cursor-pointer"
                >
        ...
        {/* Tabla de eventos de notificación (dinámica) */}
        <div
          className={`rounded-2xl p-6 ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.sombra}`}
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${tema.colores.textoSecundario}`}>
                  Cada fila representa un tipo de evento real configurado en tu
                  sistema. Todo se alimenta directamente desde tu base de datos.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className={tema.colores.textoSecundario}>
                Para agregar o eliminar tipos de evento, usa el módulo de
                administración global.
              </span>
            </div>
          </div>

          {eventosConfig.length === 0 ? (
            <div className="py-10 text-center">
              <AlertOctagon
                className={`w-10 h-10 mx-auto mb-3 ${tema.colores.textoSecundario}`}
              />
              <p className={`text-sm ${tema.colores.textoSecundario}`}>
                Aún no hay tipos de evento configurados para notificaciones. Pídele
                a un administrador que los habilite en el panel maestro.
              </p>
            </div>
          ) : (
            <div className="overflow-x-auto custom-scrollbar">
              <table className="min-w-full text-xs md:text-sm">
                <thead>
                  <tr
                    className={`text-[11px] uppercase tracking-wide ${tema.colores.textoSecundario}`}
                  >
                    ...
                    <th className="py-3 px-3 text-center">Push</th>
                    ...
                    <th className="py-3 px-3 text-center">Crítico</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-700/40">
                  {eventosConfig.map((evento) => (
                    <tr key={evento.id_tipo_evento} className={tema.colores.hover}>
                      <td className="py-3 px-3 align-top">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            ...
                            {evento.critico && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-500/15 text-red-300 border border-red-500/40">
                                CRÍTICO
                              </span>
                            )}
                            {!evento.activo && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-gray-500/10 text-gray-300 border border-gray-500/40">
                                INACTIVO
                              </span>
                            )}
                          </div>
                          <p
                            className={`text-[11px] max-w-xs ${tema.colores.textoSecundario}`}
                          >
                            {evento.descripcion}
                          </p>
                          <p className="text-[11px] text-indigo-300">
                            Código: <span className="font-mono">{evento.codigo}</span>
                          </p>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3 text-center align-middle">
                        <button
                          type="button"
                          onClick={() =>
                            handleToggleCanalEvento(evento.id_tipo_evento, "email")
                          }
                        >
                          {renderToggleMini(evento.canales.email)}
                        </button>
                      </td>
                      {/* Push */}
                      <td className="py-3 px-3 text-center align-middle">
                        <button
                          type="button"
                          onClick={() =>
                            handleToggleCanalEvento(evento.id_tipo_evento, "push")
                          }
                        >
                          {renderToggleMini(evento.canales.push)}
                        </button>
                      </td>
                      {/* SMS */}
                      <td className="py-3 px-3 text-center align-middle">
                        <button
                          type="button"
                          onClick={() =>
                            handleToggleCanalEvento(evento.id_tipo_evento, "sms")
                          }
                        >
                          {renderToggleMini(evento.canales.sms)}
                        </button>
                      </td>
                      {/* WhatsApp */}
                      <td className="py-3 px-3 text-center align-middle">
                        <button
                          type="button"
                          onClick={() =>
                            handleToggleCanalEvento(
                              evento.id_tipo_evento,
                              "whatsapp"
                            )
                          }
                        >
                          {renderToggleMini(evento.canales.whatsapp)}
                        </button>
                      </td>
                      {/* Anticipación */}
                      <td className="py-3 px-3 text-center align-middle">
                        <div className="flex flex-col items-center gap-2">
                          <input
                            ...
                            min={0}
                            max={1440}
                            className={`w-20 px-2 py-1 rounded-lg text-[11px] font-semibold ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto}`}
                            value={evento.minutos_anticipacion ?? ""}
                            onChange={(e) =>
                              handleCambioAnticipacionEvento(
                                evento.id_tipo_evento,
                                e.target.value === ""
                                  ? 0
                                  : Math.max(
                      ...
                      <td className="py-3 px-3 text-center align-middle">
                        {evento.critico ? (
                          <AlertTriangle className="w-4 h-4 text-red-400 inline-block" />
                        ) : (
                          <Check className="w-4 h-4 text-emerald-300 inline-block opacity-50" />
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        {/* Horarios de silencio + Resúmenes automáticos */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {/* Horarios de silencio */}
          <div
            className={`rounded-2xl p-6 ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.sombra}`}
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className={`text-xs ${tema.colores.textoSecundario}`}>
                  Define franjas horarias donde el sistema reduce o silencia
                  notificaciones.
                </p>
              </div>
            </div>

            {horariosSilencio.length === 0 ? (
              <div className="py-8 text-center">
                <Clock3
                  className={`w-10 h-10 mx-auto mb-3 ${tema.colores.textoSecundario}`}
                />
                <p className={`text-sm ${tema.colores.textoSecundario}`}>
                  Aún no hay horarios de silencio configurados. Puedes crearlos desde
                  el módulo de administración central para toda la comuna.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {horariosSilencio.map((h) => (
                  <div
                    key={h.id_silencio}
                    className={`rounded-xl p-4 ${tema.colores.card} ${tema.colores.borde} border flex flex-col gap-2`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => handleToggleHorarioActivo(h.id_silencio)}
                        className="flex items-center gap-2 text-[11px] font-semibold"
                      >
                        {renderToggleMini(h.activo)}
                        <span
                          className={
                            h.activo ? "text-emerald-300" : "text-gray-400"
                          }
                        >
                    ...
                    <div className="flex items-center gap-3 text-[11px]">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          handleToggleCanalSilencio(h.id_silencio, "email")
                        }
                        className="flex items-center gap-1"
                      >
                        <Mail
                          className={`w-3 h-3 ${
                            h.aplica_email ? "text-emerald-300" : "text-gray-500"
                          }`}
                        />
                        <span
                          className={
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          handleToggleCanalSilencio(h.id_silencio, "push")
                        }
                        className="flex items-center gap-1"
                      >
                        <Bell
                          className={`w-3 h-3 ${
                            h.aplica_push ? "text-emerald-300" : "text-gray-500"
                          }`}
                        />
                        <span
                          className={
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          handleToggleCanalSilencio(h.id_silencio, "sms")
                        }
                        className="flex items-center gap-1"
                      >
                        <Phone
                          className={`w-3 h-3 ${
                            h.aplica_sms ? "text-emerald-300" : "text-gray-500"
                          }`}
                        />
                        <span
                          className={
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          handleToggleCanalSilencio(h.id_silencio, "whatsapp")
                        }
                        className="flex items-center gap-1"
                      >
                        <MessageSquare
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className={`text-xs ${tema.colores.textoSecundario}`}>
                  Recibe un resumen consolidado de tu jornada directamente en tu
                  correo corporativo.
                </p>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="flex items-center justify-between gap-3 mb-3">
                    ...
                    <div
                      onClick={() =>
                        handleActualizarResumenNotificaciones({
                          resumen_diario_activado:
                            !resumenNotificaciones.resumen_diario_activado,
                        })
                      }
                      className="cursor-pointer"
                    >
                      {renderToggle(resumenNotificaciones.resumen_diario_activado)}
                    </div>
                  </div>
                  <p className={`text-xs ${tema.colores.textoSecundario} mb-3`}>
                    Envía un correo al final del día con citas atendidas, tareas y
                    mensajes clave.
                  </p>
                  <div className="flex items-center gap-2 text-xs">
                    <span className={tema.colores.textoSecundario}>
                    ...
                    <input
                      type="time"
                      className={`px-2 py-1 rounded-lg text-[11px] font-semibold ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto}`}
                      value={resumenNotificaciones.resumen_diario_hora}
                      onChange={(e) =>
                        handleActualizarResumenNotificaciones({
                          resumen_diario_hora: e.target.value,
                        })
                      }
                    />
                  </div>
                ...
                >
                  <div className="flex items-center justify-between gap-3 mb-3">
                    ...
                    <div
                      onClick={() =>
                        handleActualizarResumenNotificaciones({
                          resumen_semanal_activado:
                            !resumenNotificaciones.resumen_semanal_activado,
                        })
                      }
                      className="cursor-pointer"
                    >
                      {renderToggle(resumenNotificaciones.resumen_semanal_activado)}
                    </div>
                  </div>
                  <p className={`text-xs ${tema.colores.textoSecundario} mb-3`}>
                    Reporte consolidado de productividad, ausencias y carga del
                    centro.
                  </p>
                  <div className="flex items-center gap-3 text-xs">
                    <div className="flex items-center gap-2">
                      <span className={tema.colores.textoSecundario}>Día:</span>
                      <select
                        className={`px-2 py-1 rounded-lg text-[11px] font-semibold ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto}`}
                        value={resumenNotificaciones.resumen_semanal_dia}
                        onChange={(e) =>
                          handleActualizarResumenNotificaciones({
                            resumen_semanal_dia: Number(e.target.value),
                          })
                        }
                      >
                    ...
                    <div className="flex items-center gap-2">
                      <span className={tema.colores.textoSecundario}>Hora:</span>
                      <input
                        type="time"
                        className={`px-2 py-1 rounded-lg text-[11px] font-semibold ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto}`}
                        value={resumenNotificaciones.resumen_semanal_hora}
                        onChange={(e) =>
                          handleActualizarResumenNotificaciones({
                            resumen_semanal_hora: e.target.value,
                          })
                        }
                      />
                    </div>
                ...
                {/* Estado actual */}
                <div
                  className={`rounded-2xl p-4 ${tema.colores.card} ${tema.colores.borde} border flex items-center justify-between mt-2`}
                >
                  <div className="flex items-center gap-2 text-[11px]">
                    <Sparkles className="w-4 h-4 text-emerald-300" />
                    <span className={tema.colores.textoSecundario}>
                      Tus resúmenes se generan en segundo plano utilizando sólo datos
                      reales de tu centro.
                    </span>
                  </div>
                  {saving && (
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex-1 max-w-2xl">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar opciones de notificación, plantillas, ayuda..."
                value={busqueda}
      ...
      >
        {/* Encabezado Configuración Notificaciones */}
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              >
                Configuración · Notificaciones inteligentes
              </p>
              <h2
              ...
              <p
                className={`text-sm font-semibold ${tema.colores.textoSecundario}`}
              >
                Desde aquí controlas todos los canales, eventos y horarios de tu
                centro de alertas. Todo se alimenta desde tu base de datos real.
              </p>
            </div>
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              {mensajeEstado && (
                ...
                >
                  {tipoMensajeEstado === "ok" && <CheckCircle2 className="w-4 h-4" />}
                  {tipoMensajeEstado === "error" && (
