// frontend/src/app/(dashboard)/secretaria/perfil/page.tsx
"use client";
...
interface SecretariaLaboral {
  id_secretaria: number;
  estado?: string;
  jornada?: string;
  extension_telefonica?: string | null;
  centro?: CentroPrincipal | null;
}
...
interface UsuarioSesion {
  ...
  centro_principal?: CentroPrincipal | null;
  secretaria?: SecretariaLaboral;
}
...
const ENDPOINTS = {
  metricas: "/api/secretaria/metricas",
  actividad: "/api/secretaria/actividad-reciente",
  disponibilidad: "/api/secretaria/disponibilidad",
  documentos: "/api/secretaria/documentos",
  integraciones: "/api/secretaria/integraciones",
  auditoria: "/api/secretaria/auditoria?limit=5",
};
...
const MobileBottomBar = ({
  ...
  const effectiveBase = basePath || "/secretaria";

  const items = [
    { label: "Inicio", icon: Home, href: effectiveBase },
    { label: "Agenda", icon: Calendar, href: `${effectiveBase}/agenda` },
    { label: "Llamadas", icon: Phone, href: `${effectiveBase}/llamadas` },
    { label: "Perfil", icon: User, href: `${effectiveBase}/perfil` },
  ];
...
// PAGE
// ===============
export default function PerfilSecretariaPage() {
  ...
  const [seccionActiva, setSeccionActiva] = useState<
    "personal" | "laboral" | "preferencias" | "publico" | "seguridad" | "documentos" | "integraciones"
  >("personal");
  ...
  const centroPrincipal: CentroPrincipal | null =
    usuario?.secretaria?.centro || usuario?.centro_principal || null;

  // =============== cargar sesión (cualquier tipo de usuario)
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        if (data.success && data.usuario) {
          const u: UsuarioSesion = data.usuario;
          setUsuario(u);
          ...
          // Datos laborales de secretaria
          const sec = u.secretaria;
          const centro = sec?.centro || u.centro_principal || null;

          setFormLaboral({
            estado: sec?.estado || "activo",
            jornada: sec?.jornada || "completo",
            extension_telefonica: sec?.extension_telefonica || "",
            centro_nombre: centro?.nombre || "",
            centro_ciudad: centro?.ciudad || "",
            centro_region: centro?.region || "",
          });
        } else {
  ...
  useEffect(() => {
    const cargarTema = async () => {
      try {
        const local = typeof window !== "undefined" ? localStorage.getItem("tema_usuario") : null;
  ...
  () => [
    { titulo: "Dashboard", icono: Home, url: "/secretaria" },

    { titulo: "Agenda", icono: Calendar, url: "/secretaria/agenda" },

    { titulo: "Confirmaciones", icono: ClipboardCheck, url: "/secretaria/confirmaciones" },
    ...
    { titulo: "Recordatorios", icono: Bell, url: "/secretaria/recordatorios" },

    { titulo: "Documentos", icono: FileText, url: "/secretaria/documentos" },

    { titulo: "Mensajes", icono: MessageSquare, url: "/secretaria/mensajes" },

    { titulo: "Telemedicina", icono: Video, url: "/secretaria/telemedicina" },

    { titulo: "Tareas", icono: ClipboardList, url: "/secretaria/tareas" },

    { titulo: "Reportes", icono: BarChart3, url: "/secretaria/reportes" },
  ...
  const guardarPerfil = async () => {
    ...
    try {
      const res = await fetch("/api/secretaria/perfil", {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  // base para bottom bar (para cualquier usuario)
  let basePath = "/dashboard";
  if (usuario?.rol?.nombre) {
    const rolNorm = usuario.rol.nombre
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .toUpperCase();
    if (rolNorm.includes("SECRETARIA")) basePath = "/secretaria";
    else if (rolNorm.includes("MEDICO")) basePath = "/medico";
    else if (rolNorm.includes("ADMIN")) basePath = "/admin";
  }
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <p className={`${tema.colores.texto} text-lg font-semibold`}>Cargando perfil...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${tema.colores.fondo} pb-14 md:pb-0`}>
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-700/30">
            {sidebarAbierto ? (
              <div className="flex items-center gap-3">
                {centroPrincipal?.logo_url ? (
                  <Image
                    src={centroPrincipal.logo_url}
                    alt={centroPrincipal.nombre}
                    width={56}
                    height={56}
                ...
                <div>
                  <p className={`text-sm font-bold ${tema.colores.texto}`}>
                    {centroPrincipal?.nombre || "Mi organización"}
                  </p>
                  {centroPrincipal && (
              ...
              <div className="mx-auto">
                <ClipboardList className={`w-7 h-7 ${tema.colores.acento}`} />
              </div>
            )}
                ...
                <div className="relative">
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <item.icono
                        className={`w-5 h-5 ${
                          (item as any).activo ? "text-white" : tema.colores.acento
                        }`}
                      />
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <ClipboardList className={`w-7 h-7 ${tema.colores.acento}`} />
                <p className={`${tema.colores.texto} font-bold`}>Menú principal</p>
              </div>
        ...
        <div className="flex items-center justify-between px-4 md:px-8 py-3 gap-4">
          ...
          <div className="flex-1 max-w-2xl">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar personas, citas o configuraciones..."
                ...
                >
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    ...
                    <div className={`p-3 rounded-xl ${tema.colores.hover} flex gap-2`}>
                      ...
                      <div>
                        <p className={`text-xs ${tema.colores.texto}`}>
                          Nuevo mensaje interno recibido
                        </p>
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className={tema.colores.textoSecundario}>
              Administra tu identidad, tus datos laborales y el comportamiento de tu panel.
            </p>
          </div>
          ...
          {cargandoMetricas ? (
            ...
            <>
              ...
              <Estadistica
                ...
                secondary="Agendadas o reprogramadas"
                tema={tema}
              />
              <Estadistica
                icon={ActivityIcon}
                ...
                value={
                  metricas ? `${metricas.tasa_asistencia.toFixed(1)}%` : `${(0).toFixed(1)}%`
                }
                secondary="Confirmadas vs. agendadas"
                tema={tema}
              />
              <Estadistica
                icon={Shield}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-1 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                ...
                <div className="flex-1">
                  <p className={`text-sm ${tema.colores.textoSecundario} mb-2`}>
                    Se mostrará en panel, módulos, llamadas y auditoría.
                  </p>
            ...
            >
              <p className="text-sm font-bold text-white/80 mb-2">Resumen del perfil</p>
              ...
              <div className="flex flex-wrap items-center gap-3 mb-3">
                <span className="text-white text-sm font-semibold flex items-center gap-1">
                  ...
                  Secretaría · Gestión de agenda
                </span>
                {formLaboral.extension_telefonica && (
                  <span className="text-white/80 text-xs flex items-center gap-1">
                    <Phone className="w-3 h-3" />
                    Anexo {formLaboral.extension_telefonica}
                  </span>
                )}
                <span className="text-white/80 text-xs flex items-center gap-1">
                  <Clock className="w-3 h-3" />
                  Jornada:{" "}
                  {formLaboral.jornada === "completo"
                    ? "Completa"
                    : formLaboral.jornada === "manana"
                    ? "Mañana"
                    : formLaboral.jornada === "tarde"
                    ? "Tarde"
                    : formLaboral.jornada === "noche"
                    ? "Noche"
                    : formLaboral.jornada}
                </span>
              </div>
            ...
            >
              ...
              <div className="space-y-3 mb-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <p className={tema.colores.texto}>Documentos laborales</p>
                    <p className={`text-xs ${tema.colores.textoSecundario}`}>
                      Falta subir documentación respaldatoria
                    </p>
                  </div>
                  <button
                    onClick={() => setSeccionActiva("documentos")}
              ...
              >
                <div>
                  ...
                  <p className={`text-xs ${tema.colores.textoSecundario}`}>Centro / cuenta asignada</p>
                </div>
                <Link href="/secretaria/configuracion" className="text-xs text-indigo-400">
                  Ver límites
                </Link>
              </div>
            </div>
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                { id: "laboral", label: "Laboral", icon: ShieldCheck },
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className={tema.colores.textoSecundario}>
                      Datos del cargo, jornada y centro donde trabajas.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      value={formLaboral.extension_telefonica}
                      onChange={(e) =>
                        setFormLaboral((p) => ({
                          ...p,
                  ...
                  <div>
                    ...
                    <input
                      value={formLaboral.centro_nombre}
                      readOnly
                      className={`w-full mt-1 px-4 py-3 rounded-xl ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto} opacity-80`}
                    />
                  </div>
                  <div>
                    <label className={`text-xs font-semibold ${tema.colores.textoSecundario}`}>
                    ...
                    <input
                      value={formLaboral.centro_ciudad}
                      readOnly
                      className={`w-full mt-1 px-4 py-3 rounded-xl ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto} opacity-80`}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      value={formLaboral.centro_region}
                      readOnly
                      className={`w-full mt-1 px-4 py-3 rounded-xl ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto} opacity-80`}
                    />
                  </div>
                  ...
                  <div className="mt-4 p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/20 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                    ...
                    <div className="flex-1">
                      ...
                      <p className={`text-sm font-semibold ${tema.colores.texto}`}>
                        {centroPrincipal.nombre} · {centroPrincipal.ciudad}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-6">
                  <p className={`text-sm font-bold mb-3 ${tema.colores.texto}`}>
                    Disponibilidad de agenda (preview)
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {cargandoDisponibilidad ? (
                      ...
                      <p className={tema.colores.textoSecundario}>
                        Aún no has configurado la disponibilidad de agenda.
                      </p>
                    )}
                  </div>
                </div>
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className={tema.colores.textoSecundario}>
                      Personaliza la experiencia de tu panel.
                    </p>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className={tema.colores.textoSecundario}>
                      Esto puede mostrarse en tu ficha interna.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Ej: Secretaria con experiencia en gestión de agendas y atención de público..."
                    ></textarea>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className={tema.colores.textoSecundario}>
                      Sube tus contratos, certificados y permisos.
                    </p>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className={tema.colores.textoSecundario}>
                      Conecta calendarios y videollamadas.
                    </p>
                  </div>
                </div>
                  ...
                  <div className={`p-4 rounded-xl ${tema.colores.secundario} flex items-start gap-3`}>
                    ...
                    <div>
                      ...
                      <p className={`text-xs ${tema.colores.textoSecundario} mb-2`}>
                        Recomendado para proteger tu cuenta
                      </p>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={tema.colores.textoSecundario}>Perfil de la secretaría</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <Link href="/ayuda" className={tema.colores.textoSecundario}>
      ...
      {/* bottom bar mobile */}
      <MobileBottomBar tema={tema} actual={`${basePath}/perfil`} basePath={basePath} />
