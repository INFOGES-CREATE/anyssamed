// src/app/(dashboard)/secretaria/telemedicina/page.tsx
...
export default function TelemedicinaSecretariaPage() {
  ...
  // cuando hay usuario secretaria, cargo datos
  useEffect(() => {
    if (usuario) {
      cargarSesiones();
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("tema_telemedicina_secretaria", temaActual);
      localStorage.setItem("vista_telemedicina_secretaria", tipoVista);
      localStorage.setItem(
        "sidebar_secretaria_telemedicina_abierto",
        JSON.stringify(sidebarAbierto)
      );
    }
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const temaGuardado = localStorage.getItem(
        "tema_telemedicina_secretaria"
      );
      const vistaGuardada = localStorage.getItem(
        "vista_telemedicina_secretaria"
      );
      const sidebarGuardado = localStorage.getItem(
        "sidebar_secretaria_telemedicina_abierto"
      );
  ...
  const cargarDatosUsuario = async () => {
    try {
      ...
      if (result.success && result.usuario) {
        ...
        const tienePermisoSecretaria = rolesUsuario.some((rol) =>
          [
            "SECRETARIA",
            "ADMINISTRATIVO",
            "ADMINISTRACION",
            ...
            "COORDINADOR",
            "GESTION",
          ].some((clave) => rol.includes(clave))
        );

        if (!tienePermisoSecretaria) {
          agregarNotificacion({
            tipo: "error",
            titulo: "Acceso Denegado",
            mensaje:
              "Este módulo de telemedicina es exclusivo para secretarías y administrativos.",
          });
          setTimeout(() => {
        ...
        agregarNotificacion({
          tipo: "success",
          titulo: "Bienvenida a Telemedicina",
          mensaje: `${result.usuario.nombre} ${result.usuario.apellido_paterno}, ya puedes gestionar las sesiones de tu centro.`,
        });
      } else {
  ...
  const cargarSesiones = async () => {
    ...
    try {
      ...
      if (result.success && result.sesiones) {
        ...
        if (sesionesEnEspera.length > sesionesEnEsperaAnterior.length) {
          agregarNotificacion({
            ...
            mensaje:
              "Hay nuevos pacientes en sala de espera para sus sesiones de telemedicina.",
            accion: { texto: "Ver Sesiones", url: "#sesiones" },
          });
      ...
      agregarNotificacion({
        ...
        mensaje: "No se pudieron cargar las sesiones de telemedicina.",
      });
    } finally {
  ...
  const cargarNotificacionesIniciales = () => {
    if (typeof window !== "undefined") {
      const notificacionesGuardadas = localStorage.getItem(
        "notificaciones_telemedicina_secretaria"
      );
  ...
  const verificarNuevasNotificaciones = async () => {
    // placeholder para futura integración con backend de notificaciones
  };
  ...
  const entrarSala = async (sesion: SesionTelemedicina) => {
    try {
      agregarNotificacion({
        ...
        mensaje:
          "Generando acceso seguro a la sala de telemedicina para gestionar al paciente...",
      });
      ...
      if (result.success && result.acceso) {
        agregarNotificacion({
          ...
          mensaje: "Abriendo sala de telemedicina...",
        });
        setTimeout(() => {
      ...
      agregarNotificacion({
        ...
        mensaje:
          error.message ||
          "No se pudo acceder a la sala. Verifica con soporte técnico.",
      });
    }
  };
  ...
  const exportarDatos = () => {
    const datos = {
      ...
      generado_por: {
        ...
        rol: usuario?.rol?.nombre || "Secretaría / Administrativo",
      },
    };
    ...
    a.href = url;
    a.download = `telemedicina-secretaria-${new Date()
      .toISOString()
      .split("T")[0]}.json`;
    a.click();
    agregarNotificacion({
      ...
      mensaje:
        "Los datos de telemedicina del centro se han descargado correctamente.",
    });
  };

  const crearNuevaSesion = () =>
    router.push("/secretaria/telemedicina/nueva");
  const verAgenda = () => router.push("/secretaria/agenda");
  const verHistorial = () => router.push("/secretaria/telemedicina/historial");
  const abrirConfiguracion = () =>
    router.push("/secretaria/configuracion/telemedicina");

  const enviarMensajePaciente = (sesion: SesionTelemedicina) => {
    if (sesion.paciente.email) {
      window.location.href = `mailto:${sesion.paciente.email}?subject=Recordatorio%20telemedicina&body=Hola%20${encodeURIComponent(
        sesion.paciente.nombre_completo
      )}%0D%0A%0D%0ALe recordamos su sesión de telemedicina programada.`;
      return;
    }
  ...
  const cancelarSesion = async (sesion: SesionTelemedicina) => {
    try {
      const confirmar = window.confirm(
        "¿Seguro que deseas cancelar esta sesión de telemedicina?"
      );
      if (!confirmar) return;
  ...
  const limpiarNotificaciones = () => {
    setNotificaciones([]);
    if (typeof window !== "undefined") {
      localStorage.removeItem("notificaciones_telemedicina_secretaria");
    }
  };
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Cargando Telemedicina Secretaría
          </motion.h2>
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            Preparando el panel para gestionar las sesiones del centro...
          </motion.p>
        </motion.div>
      </div>
    );
      ...
      >
        ...
        >
          ...
          <p className={`text-lg mb-8 ${tema.colores.textoSecundario}`}>
            No tienes permisos para acceder al panel de telemedicina de
            secretaría.
          </p>
          <Link
            href="/login"
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className={`text-2xl font-black ${tema.colores.texto}`}>
                  Telemedicina – Secretaría
                </h1>
                <p className={`text-sm ${tema.colores.textoSecundario}`}>
                  Gestión en tiempo real de las sesiones de tu centro
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  ...
                  <p className={`text-xs ${tema.colores.textoSecundario}`}>
                    {usuario.rol?.nombre || "Secretaría / Administrativo"}
                  </p>
                </div>
                <motion.div
              ...
              <div className="p-6 space-y-6 overflow-y-auto h-full custom-scrollbar">
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      ...
                      <motion.div
                        ...
                        onClick={() => router.push("/secretaria/evaluaciones")}
                      >
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-xs ${tema.colores.textoSecundario}`}
                ...
                >
                  ...
                  <p className="text-xs text-white/80 mb-3">
                    ¿Problemas con la conexión o una sala de telemedicina?
                  </p>
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() =>
                      window.open(
                        "mailto:soporte@anyssa.cl?subject=Soporte%20Telemedicina%20Secretar%C3%ADa",
                        "_blank"
                      )
                    }
                ...
                estadisticas && (
                  ...
                  >
                    ...
                    <motion.div
                      whileHover={{ scale: 1.05, y: -5 }}
                      className={`rounded-2xl ${tema.colores.card} ${tema.colores.borde} border p-6 ${tema.colores.sombra} cursor-pointer`}
                      onClick={() => router.push("/secretaria/finanzas")}
                    >
                      <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className={`text-2xl font-black ${tema.colores.texto}`}>
                    Sesiones de Telemedicina del Centro
                  </h2>
                  <p className={`text-sm ${tema.colores.textoSecundario}`}>
                          ...
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <div className="flex items-start gap-4 flex-1">
                              <motion.div
                                ...
                                onClick={() =>
                                  router.push(
                                    `/secretaria/pacientes/${sesion.id_paciente}`
                                  )
                                }
                              >
                            ...
                            <div className="flex flex-col gap-2 ml-4">
                              ...
                              {sesion.estado === "finalizada" && (
                                <motion.button
                                  ...
                                  onClick={() =>
                                    router.push(
                                      `/secretaria/telemedicina/pacientes/${sesion.id_paciente}/historial`
                                    )
                                  }
        ...
        <div className="max-w-[1920px] mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className={`text-sm font-bold ${tema.colores.texto}`}>
                  AnySSA Telemedicina – Secretaría
                </p>
                <p className={`text-xs ${tema.colores.textoSecundario}`}>
