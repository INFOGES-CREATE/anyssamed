// src/app/(dashboard)/tecnico/configuracion/accesibilidad/page.tsx
"use client";
...
interface ConfiguracionAccesibilidadRegistro {
...
interface ConfiguracionAccesibilidadResponse {
  success: boolean;
  configuraciones: ConfiguracionAccesibilidadRegistro[];
}
...
function SunLike(props: any) {
  return <Globe {...props} />;
}
function MoonLike(props: any) {
  return <Wifi {...props} />;
}
...
export default function ConfiguracionAccesibilidadCentroPage() {
  ...
  const [configuraciones, setConfiguraciones] = useState<
    ConfiguracionAccesibilidadRegistro[]
  >([]);
  ...
  const gruposConfiguraciones = useMemo(() => {
    const grupos = new Map<string, ConfiguracionAccesibilidadRegistro[]>();
  ...
  useEffect(() => {
    if (usuario?.tecnico) {
      ...
      cargarConfiguracionAccesibilidad();
    }
  }, [usuario]);
  ...
  const cargarDatosUsuario = async () => {
    try {
      ...
      if (result.success && result.usuario) {
        ...
        if (!tieneRolTecnico) {
          alert(
            `Acceso denegado. Esta configuración de accesibilidad es solo para técnicos. Tus roles actuales son: ${rolesUsuario.join(
              ", "
            )}`
          );
  ...
  const cargarConfiguracionAccesibilidad = async () => {
    if (!usuario?.tecnico?.id_tecnico) return;
    ...
    try {
      ...
      const res = await fetch(
        `/api/tecnico/configuracion/accesibilidad?${params.toString()}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      const data = (await res.json().catch(() => ({}))) as
        | ConfiguracionAccesibilidadResponse
        | any;

      if (!res.ok || !data?.success) {
        console.warn(
          "No se encontró configuración de accesibilidad, la lista se mostrará vacía",
          data
        );
        setConfiguraciones([]);
        setConfigOriginal([]);
      ...
      const items =
        (data.configuraciones || []) as ConfiguracionAccesibilidadRegistro[];
      ...
      setErrorConfig(
        "No se pudo cargar la configuración de accesibilidad del centro. Revisa la conexión."
      );
      setConfiguraciones([]);
      setConfigOriginal([]);
    } finally {
  ...
  const actualizarValorConfiguracion = (
    target: ConfiguracionAccesibilidadRegistro,
  ...
  const guardarConfiguracionAccesibilidad = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/tecnico/configuracion/accesibilidad", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
      ...
      if (!res.ok || !data?.success) {
        console.error("Error al guardar configuración accesibilidad:", data);
        setErrorConfig(
          data?.message ||
            "No se pudo guardar la configuración de accesibilidad. Inténtalo nuevamente."
        );
        return;
      }
      ...
      const nuevas =
        (data.configuraciones ||
          configuraciones) as ConfiguracionAccesibilidadRegistro[];

      setConfiguraciones(nuevas);
      setConfigOriginal(nuevas);
      setMensajeConfig(
        `Configuración de accesibilidad del centro guardada correctamente.`
      );
    } catch (error) {
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          >
            Preparando preferencias de visualización y experiencia de uso para tu
            centro...
          </p>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <p className={`text-lg mb-8 ${tema.colores.textoSecundario}`}>
            No tienes permisos para acceder a la configuración de accesibilidad del
            centro.
          </p>
          <Link
            href="/login"
        ...
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex-1 max-w-2xl">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar opciones de accesibilidad por clave, descripción o grupo..."
                value={busqueda}
      ...
      >
        ...
        <div className="mb-8 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            >
              Ajusta cómo se ve y se siente el sistema para tu centro: accesibilidad,
              legibilidad y ayudas visuales.
            </p>
          ...
          <div className="flex flex-col items-end gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <button
                onClick={cargarConfiguracionAccesibilidad}
              ...
              <button
                onClick={guardarConfiguracionAccesibilidad}
            ...
            <div className="text-xs md:text-sm text-right space-y-1">
              {ultimaActualizacion ? (
                ...
                <p className={tema.colores.textoSecundario}>
                  Aún no hay registros de accesibilidad definidos para este centro.
                </p>
              )}
              {hayCambios && (
        ...
        {loadingConfig ? (
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p
                className={`text-lg font-semibold ${tema.colores.textoSecundario}`}
              >
                Cargando parámetros de accesibilidad configurables...
              </p>
            </div>
          </div>
          ...
          >
            <p className={`text-lg font-semibold ${tema.colores.texto}`}>
              No hay parámetros de accesibilidad definidos para este centro aún.
            </p>
            <p className={tema.colores.textoSecundario}>
              Una vez que se creen claves en la tabla de accesibilidad del centro,
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        Ajustes de accesibilidad definidos para el grupo "
                        {grupo.nombre}".
                      </p>
                    </div>
                  </div>
        ...
        >
          <div className="text-xs md:text-sm">
            <p className={tema.colores.textoSecundario}>
              Esta página consume{" "}
              <span className={tema.colores.texto}>
                claves dinámicas de accesibilidad del centro
              </span>
              . Cada vez que se agreguen o modifiquen claves en la base de datos,
              aparecerán automáticamente agrupadas aquí sin cambiar el código
              frontend.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
