// src/app/(dashboard)/tecnico/configuracion/general/page.tsx
"use client";
...
interface ConfiguracionCentroRegistro {
  id_configuracion: number | null;
  id_centro: number;
  clave: string;
  valor: string; // se guarda como string; el tipo lo indica tipo_dato
...
interface ConfiguracionGeneralResponse {
  success: boolean;
  configuraciones: ConfiguracionCentroRegistro[];
}
...
const TEMAS: Record<TemaColor, ConfiguracionTema> = {
  ...
  green: {
    ...
    icono: Smartphone, // solo para variar icono
    colores: {
...
export default function ConfiguracionGeneralCentroPage() {
  ...
  const gruposConfiguraciones = useMemo(() => {
    const grupos = new Map<string, ConfiguracionCentroRegistro[]>();
  ...
  const cargarDatosUsuario = async () => {
    try {
      ...
      if (result.success && result.usuario) {
        ...
        if (!tieneRolTecnico) {
          alert(
            `Acceso denegado. Esta configuración general es solo para técnicos. Tus roles actuales son: ${rolesUsuario.join(
              ", "
            )}`
          );
  ...
  const cargarConfiguracionGeneral = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/tecnico/configuracion/general?${params.toString()}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!res.ok || !data?.success) {
        console.warn(
          "No se encontró configuración general, la lista se mostrará vacía",
          data
        );
        setConfiguraciones([]);
        setConfigOriginal([]);
      ...
      const items = (data.configuraciones || []) as ConfiguracionCentroRegistro[];
      ...
      setErrorConfig(
        "No se pudo cargar la configuración general del centro. Revisa la conexión."
      );
      setConfiguraciones([]);
      setConfigOriginal([]);
    } finally {
  ...
  const actualizarValorConfiguracion = (
    target: ConfiguracionCentroRegistro,
  ...
  const guardarConfiguracionGeneral = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/tecnico/configuracion/general", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
      ...
      if (!res.ok || !data?.success) {
        console.error("Error al guardar configuración general:", data);
        setErrorConfig(
          data?.message ||
            "No se pudo guardar la configuración general. Inténtalo nuevamente."
        );
        return;
      }

      const nuevas = (data.configuraciones ||
        configuraciones) as ConfiguracionCentroRegistro[];

      setConfiguraciones(nuevas);
      setConfigOriginal(nuevas);
      setMensajeConfig(
        `Configuración general del centro guardada correctamente.`
      );
    } catch (error) {
      ...
      >
        <div className="text-center">
          ...
          <h2 className={`text-4xl font-black mb-4 ${tema.colores.texto}`}>
            Cargando Configuración General del Centro
          </h2>
          <p
            className={`text-lg font-semibold ${tema.colores.textoSecundario} animate-pulse`}
          >
            Levantando parámetros generales y preferencias de este
            establecimiento...
          </p>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <p className={`text-lg mb-8 ${tema.colores.textoSecundario}`}>
            No tienes permisos para acceder a la configuración general del centro.
          </p>
          <Link
            href="/login"
        ...
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex-1 max-w-2xl">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar por clave, descripción o grupo dentro de la configuración general..."
                value={busqueda}
      ...
      >
        ...
        <div className="mb-8 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            >
              Ajusta los parámetros generales de funcionamiento del centro sin
              tocar la configuración global del sistema.
            </p>
          ...
          <div className="flex flex-col items-end gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <button
                onClick={cargarConfiguracionGeneral}
            ...
            <div className="text-xs md:text-sm text-right space-y-1">
              {ultimaActualizacion ? (
                ...
                <p className={tema.colores.textoSecundario}>
                  Aún no hay registros de configuración general para este centro.
                </p>
              )}
              {hayCambios && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6 mb-8">
          <ResumenCard
            tema={tema}
            icono={Globe}
            titulo="Parámetros totales"
            valor={resumen.total}
            chip="Claves configuradas"
            color="from-indigo-500 to-cyan-500"
          />
          <ResumenCard
            tema={tema}
            icono={ClipboardLike}
            titulo="Grupos de configuración"
        ...
        {loadingConfig ? (
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p
                className={`text-lg font-semibold ${tema.colores.textoSecundario}`}
              >
                Cargando parámetros generales configurables...
              </p>
            </div>
          </div>
          ...
          >
            <p className={`text-lg font-semibold ${tema.colores.texto}`}>
              No hay parámetros de configuración general definidos para este
              centro aún.
            </p>
            <p className={tema.colores.textoSecundario}>
              Una vez que se creen claves en la tabla de configuraciones del
              centro, aparecerán automáticamente agrupadas en esta pantalla.
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        Parámetros dinámicos definidos para el grupo "
                        {grupo.nombre}".
                      </p>
                    </div>
                  </div>
        ...
        >
          <div className="text-xs md:text-sm">
            <p className={tema.colores.textoSecundario}>
              Esta página consume{" "}
              <span className={tema.colores.texto}>
                claves dinámicas desde la configuración del centro
              </span>
...
function ClipboardLike(props: any) {
  return <Activity {...props} />;
}
function LockLike(props: any) {
