...
interface ConfiguracionPreferenciaRegistro {
  ...
  grupo: string | null;
  modificable_por_centro: boolean;
  tipo_dato_detalle?: string | null;
  fecha_creacion?: string | null;
  fecha_modificacion?: string | null;
}

interface ConfiguracionPreferenciasResponse {
  success: boolean;
  configuraciones: ConfiguracionPreferenciaRegistro[];
}

// =====================================================
// TEMAS (mismos que tu página general)
// =====================================================
...
function esTruthy(valor: string | null | undefined) {
  if (!valor) return false;
  const v = valor.toString().trim().toLowerCase();
  return ["1", "true", "sí", "si", "yes", "y"].includes(v);
}
...
export default function ConfiguracionPreferenciasCentroPage() {
  ...
  const [configuraciones, setConfiguraciones] = useState<
    ConfiguracionPreferenciaRegistro[]
  >([]);
  ...
  const hayCambios = useMemo(() => {
    if (!configuraciones || !configOriginal) return false;
    return JSON.stringify(configuraciones) !== JSON.stringify(configOriginal);
  }, [configuraciones, configOriginal]);

  const ultimaActualizacion = useMemo(() => {
    if (!configuraciones || configuraciones.length === 0) return null as
      | string
      | null;
    let maxFecha: string | null = null;
    for (const c of configuraciones) {
      const f = c.fecha_modificacion || c.fecha_creacion;
      if (!f) continue;
      if (!maxFecha || new Date(f) > new Date(maxFecha)) {
        maxFecha = f;
      }
    }
    return maxFecha;
  ...
  const gruposConfiguraciones = useMemo(() => {
    const grupos = new Map<string, ConfiguracionPreferenciaRegistro[]>();

    for (const cfg of configuraciones) {
      const nombreGrupo = cfg.grupo?.trim() || "General";
      if (!grupos.has(nombreGrupo)) grupos.set(nombreGrupo, []);
      grupos.get(nombreGrupo)!.push(cfg);
    }
  ...
  const resumen = useMemo(() => {
    ...
    const grupos = new Set(
      configuraciones.map((c) => c.grupo?.trim() || "General")
    ).size;
    const editables = configuraciones.filter(
      (c) => c.modificable_por_centro
    ).length;
    const soloLectura = total - editables;
    const booleans = configuraciones.filter((c) => c.tipo_dato === "boolean")
      .length;
  ...
  useEffect(() => {
    if (usuario?.tecnico) {
      ...
      cargarConfiguracionPreferencias();
    }
  }, [usuario]);
  ...
  const cargarDatosUsuario = async () => {
    try {
      ...
      if (result.success && result.usuario) {
        ...
        if (!tieneRolTecnico) {
          alert(
            `Acceso denegado. Esta configuración de preferencias es solo para técnicos. Tus roles actuales son: ${rolesUsuario.join(
              ", "
            )}`
          );
  ...
  const cargarConfiguracionPreferencias = async () => {
    if (!usuario?.tecnico?.id_tecnico) return;
    ...
    try {
      ...
      const res = await fetch(
        `/api/tecnico/configuracion/preferencias?${params.toString()}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      const data = (await res.json().catch(() => ({}))) as
        | ConfiguracionPreferenciasResponse
        | any;

      if (!res.ok || !data?.success) {
        console.warn(
          "No se encontró configuración de preferencias, lista vacía",
          data
        );
        setConfiguraciones([]);
        setConfigOriginal([]);
      ...
      const items =
        (data.configuraciones || []) as ConfiguracionPreferenciaRegistro[];

      setConfiguraciones(items);
      setConfigOriginal(items);
    } catch (error) {
      console.error("Error al cargar configuración de preferencias:", error);
      setErrorConfig(
        "No se pudo cargar la configuración de preferencias del centro. Revisa la conexión."
      );
      setConfiguraciones([]);
      setConfigOriginal([]);
    } finally {
  ...
  const actualizarValorConfiguracion = (
    target: ConfiguracionPreferenciaRegistro,
    nuevoValor: string | boolean
  ) => {
    setConfiguraciones((prev) =>
      prev.map((item) => {
        if (
          item.id_configuracion === target.id_configuracion &&
          item.clave === target.clave
        ) {
          const val =
            typeof nuevoValor === "boolean"
              ? nuevoValor
                ? "1"
                : "0"
              : nuevoValor;
          return { ...item, valor: val };
        }
        return item;
      })
    );
  ...
  const guardarConfiguracionPreferencias = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/tecnico/configuracion/preferencias", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
      ...
      if (!res.ok || !data?.success) {
        console.error("Error al guardar configuración preferencias:", data);
        setErrorConfig(
          data?.message ||
            "No se pudo guardar la configuración de preferencias. Inténtalo nuevamente."
        );
        return;
      }
      ...
      const nuevas =
        (data.configuraciones ||
          configuraciones) as ConfiguracionPreferenciaRegistro[];

      setConfiguraciones(nuevas);
      setConfigOriginal(nuevas);
      setMensajeConfig(
        `Preferencias del centro guardadas correctamente.`
      );
    } catch (error) {
      console.error("Error al guardar configuración de preferencias:", error);
      setErrorConfig(
  ...
  const restaurarDesdeOriginal = () => {
    setConfiguraciones(configOriginal);
  };
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          >
            Preparando tus preferencias de paneles, vistas y experiencia general...
          </p>
        </div>
      </div>
    );
      ...
      >
        ...
        >
          ...
          <p className={`text-lg mb-8 ${tema.colores.textoSecundario}`}>
            No tienes permisos para acceder a las preferencias del centro.
          </p>
          <Link
            href="/login"
        ...
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex-1 max-w-2xl">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar preferencias por clave, descripción o grupo..."
                value={busqueda}
      ...
      >
        ...
        <div className="mb-8 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            >
              Ajusta las preferencias de funcionamiento y experiencia de tu centro:
              vistas por defecto, comportamientos y opciones avanzadas.
            </p>
          ...
          <div className="flex flex-col items-end gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <button
                onClick={cargarConfiguracionPreferencias}
              ...
              <button
                onClick={guardarConfiguracionPreferencias}
              ...
              >
                ...
                {guardandoConfig
                  ? "Guardando cambios..."
                  : "Guardar preferencias del centro"}
              </button>
            </div>
            ...
            <div className="text-xs md:text-sm text-right space-y-1">
              {ultimaActualizacion ? (
                ...
                <p className={tema.colores.textoSecundario}>
                  Aún no hay preferencias definidas para este centro.
                </p>
              )}
              {hayCambios && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6 mb-8">
          <ResumenCard
            tema={tema}
            icono={Smartphone}
            titulo="Preferencias totales"
            valor={resumen.total}
            chip="Parámetros activos"
            color="from-indigo-500 to-cyan-500"
          />
          <ResumenCard
            tema={tema}
            icono={Activity}
            titulo="Grupos de ajustes"
            valor={resumen.grupos}
            chip="Secciones dinámicas"
            color="from-purple-500 to-pink-500"
          />
          <ResumenCard
            tema={tema}
            icono={ShieldCheck}
            titulo="Editables por centro"
            valor={resumen.editables}
            chip="Control local"
          ...
          <ResumenCard
            tema={tema}
            icono={Clock3}
            titulo="Flags booleanos"
            valor={resumen.booleans}
            chip="Interruptores rápidos"
          ...
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p
                className={`text-lg font-semibold ${tema.colores.textoSecundario}`}
              >
                Cargando preferencias configurables del centro...
              </p>
            </div>
          </div>
        ) : gruposConfiguraciones.length === 0 ? (
          <div
            className={`rounded-2xl p-8 ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.sombra} text-center`}
          >
            <p className={`text-lg font-semibold ${tema.colores.texto}`}>
              No hay preferencias definidas para este centro aún.
            </p>
            <p className={tema.colores.textoSecundario}>
              Una vez que se creen claves en la tabla de preferencias del centro,
              aparecerán automáticamente agrupadas en esta pantalla.
            </p>
          </div>
        ) : (
          <div className="space-y-6 mb-10">
            {gruposConfiguraciones.map((grupo) => (
              <section
                key={grupo.nombre}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        Preferencias definidas para el grupo "{grupo.nombre}".
                      </p>
                    </div>
                  </div>
                  <span
                    className={`text-xs px-3 py-1 rounded-full ${tema.colores.hover} ${tema.colores.textoSecundario}`}
                  >
                    {grupo.items.length} claves
                  </span>
                </div>

                <div className="space-y-3">
                  {grupo.items.map((cfg) => {
                    const esEditable = cfg.modificable_por_centro;
                    const valorActual = cfg.valor ?? "";
                    const baseClasses =
                      "w-full px-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50";
                    const readOnlyClasses = esEditable
                      ? ""
                      : "opacity-60 cursor-not-allowed";

                    return (
                      <div
                        key={`${cfg.id_configuracion ?? "new"}-${cfg.clave}`}
                        className={`flex flex-col md:flex-row md:items-center gap-3 md:gap-4 p-3 rounded-xl ${tema.colores.hover}`}
                      >
                        <div className="md:w-1/3 space-y-1">
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-[11px] px-2 py-0.5 rounded-full bg-black/20 ${tema.colores.textoSecundario}`}
                            >
                              {cfg.tipo_dato.toUpperCase()}
                            </span>
                            {!esEditable && (
                              <span className="text-[10px] px-2 py-0.5 rounded-full bg-red-500/20 text-red-200 border border-red-400/40">
                                Solo lectura
                              </span>
                            )}
                          </div>
                          ...
                          {cfg.tipo_dato === "boolean" ? (
                            <label className="inline-flex items-center gap-2 cursor-pointer">
                              <input
                                ...
                                checked={esTruthy(valorActual)}
                                disabled={!esEditable}
                                onChange={(e) =>
                                  actualizarValorConfiguracion(cfg, e.target.checked)
                                }
                              />
                              <span
                                className={`text-xs ${tema.colores.textoSecundario}`}
                              >
                                {esEditable
                                  ? "Activar / desactivar"
                                  : "Valor controlado globalmente"}
                              </span>
                            </label>
                          ) : cfg.tipo_dato === "integer" ||
                            cfg.tipo_dato === "float" ? (
                            <input
                              type="number"
                              step={cfg.tipo_dato === "integer" ? 1 : "0.01"}
                              value={valorActual}
                              disabled={!esEditable}
                              onChange={(e) =>
                                actualizarValorConfiguracion(cfg, e.target.value)
                              }
                              className={`${baseClasses} ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto} text-right ${readOnlyClasses}`}
                            />
                          ) : cfg.tipo_dato === "datetime" ? (
                            <input
                              type="datetime-local"
                              value={
                                valorActual
                                  ? new Date(valorActual).toISOString().slice(0, 16)
                                  : ""
                              }
                              disabled={!esEditable}
                              onChange={(e) =>
                              ...
                              className={`${baseClasses} ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto} ${readOnlyClasses}`}
                            />
                          ) : cfg.tipo_dato === "json" ? (
                            <textarea
                              rows={3}
                              ...
                              className={`${baseClasses} ${tema.colores.card} ${tema.colores.borde} border ${tema.colores.texto} font-mono text-xs ${readOnlyClasses}`}
                            />
                          ) : (
                            // string por defecto
                            <input
                        ...
                        <div className="md:w-48 flex flex-col items-start md:items-end gap-1 text-[11px]">
                          {cfg.fecha_modificacion && (
                            <span className={tema.colores.textoSecundario}>
                              Modificado:{" "}
                              {formatearFechaSoloDia(cfg.fecha_modificacion)}
                            </span>
                          )}
                          {!cfg.fecha_modificacion && cfg.fecha_creacion && (
                            <span className={tema.colores.textoSecundario}>
                              Creado: {formatearFechaSoloDia(cfg.fecha_creacion)}
                            </span>
                          )}
                        </div>
                      </div>
        ...
        >
          <div className="text-xs md:text-sm">
            <p className={tema.colores.textoSecundario}>
              Esta página consume{" "}
              <span className={tema.colores.texto}>
                claves dinámicas de preferencias del centro
              </span>
              . Cada vez que se agreguen o modifiquen claves en la base de datos,
              aparecerán automáticamente agrupadas aquí sin cambios de frontend.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <button
