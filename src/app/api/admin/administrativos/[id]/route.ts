// frontend/src/app/api/admin/administrativos/[id]/route.ts
...
interface AdministrativoDetalle extends RowDataPacket {
  // ADMINISTRATIVO
  ...
  fecha_creacion_administrativo: string;
  fecha_modificacion_administrativo: string | null;
  cargo: string;
  nivel_acceso: string;
  numero_empleado?: string | null;
  descripcion?: string | null;
  total_secretarias_supervisadas?: number;
  ...
  ciudad?: string | null;
  region?: string | null;
  estado: string; // estado usuario (u.estado)
...
interface UpdateAdministrativoBody {
  // Datos usuario
  username?: string;
  nombre?: string;
  apellido_paterno?: string;
...
// ============================================================================
// GET - OBTENER DETALLE COMPLETO DEL ADMINISTRATIVO + USUARIO
// ============================================================================
...
export async function GET(
  ...
  try {
    const idAdministrativo = parseInt(params.id);

    if (isNaN(idAdministrativo)) {
      return NextResponse.json(
        { success: false, error: "ID de administrativo inválido" },
        { status: 400 }
      );
    }
    ...
    // ========== 1. DATOS PRINCIPALES ADMINISTRATIVO + USUARIO ==========
    const [administrativos] = await connection.query<
      AdministrativoDetalle[] & RowDataPacket[]
    >(
      `SELECT
        ...
        adm.estado AS estado_administrativo,
        adm.fecha_creacion AS fecha_creacion_administrativo,
        adm.fecha_modificacion AS fecha_modificacion_administrativo,
        adm.cargo,
        adm.nivel_acceso,
        adm.numero_empleado,
        adm.descripcion,
        ...
        GROUP_CONCAT(
          DISTINCT r.nombre
          ORDER BY r.nivel_jerarquia DESC SEPARATOR ', '
        ) AS roles_nombres,
        GROUP_CONCAT(
          DISTINCT r.descripcion
          ORDER BY r.nivel_jerarquia DESC SEPARATOR ' | '
        ) AS roles_descripciones,
      ...
      INNER JOIN usuarios u
        ON adm.id_usuario = u.id_usuario
      ...
      LEFT JOIN centros_medicos c
        ON adm.id_centro = c.id_centro
      LEFT JOIN sucursales s
        ON adm.id_sucursal = s.id_sucursal
    ...
    if (administrativos.length === 0) {
      connection.release();
      return NextResponse.json(
        { success: false, error: "Administrativo no encontrado" },
        { status: 404 }
      );
    }

    const administrativo = administrativos[0];
    const idUsuario = administrativo.id_usuario;

    // ========== 2. ESTADÍSTICAS (CITAS, LOGS, ETC.) ==========
    ...
    const [historialEstados] = await connection.query<RowDataPacket[]>(
      ...
      FROM logs_sistema
      WHERE id_usuario = ?
        AND modulo IN ('usuarios','administrativos')
        AND accion IN (
          'cambiar_estado',
          'bloquear_usuario',
          'desbloquear_usuario',
          'editar_usuario',
          'editar_administrativo',
          'activar_2fa',
          'desactivar_2fa',
    ...
    const administrativoLimpio = limpiarDatosSensibles(administrativo);

    return NextResponse.json({
      success: true,
      data: {
        ...administrativoLimpio,
        estadisticas:
          estadisticas[0] || {
            total_citas: 0,
            citas_completadas: 0,
    ...
    await registrarLog({
      ...
      accion: "obtener_administrativo",
      descripcion: `Error al obtener administrativo ID ${params.id}: ${error.message}`,
      nivel_severidad: 7,
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Error al obtener administrativo",
...
// ============================================================================
// PUT - ACTUALIZAR DATOS DE ADMINISTRATIVO + USUARIO
// ============================================================================
...
export async function PUT(
  ...
  try {
    ...
    const body: UpdateAdministrativoBody = await request.json();
    const {
      username,
      nombre,
      apellido_paterno,
    ...
    // ========== OBTENER DATOS ANTIGUOS (ADMINISTRATIVO + USUARIO) ==========
    const [rowsAntiguos] = await connection.query<AdministrativoDetalle[]>(
      `SELECT adm.*, u.*
       FROM administrativos adm
    ...
    const administrativoAntiguo = rowsAntiguos[0];
    const idUsuario = administrativoAntiguo.id_usuario;

    // ========== VALIDAR DUPLICADOS EN USUARIOS ==========
    if (username && username !== administrativoAntiguo.username) {
    ...
    if (email && email !== administrativoAntiguo.email) {
    ...
    if (rut && rut !== administrativoAntiguo.rut) {
    ...
    // ========== CENTRO Y SUCURSAL (COMPARTIDOS USUARIO + ADMINISTRATIVO) ==========
    const centroAsignado =
      typeof id_centro_principal === "number"
        ? id_centro_principal
        : administrativoAntiguo.id_centro;

    const sucursalAsignada =
      typeof id_sucursal_principal === "number"
        ? id_sucursal_principal
        : administrativoAntiguo.id_sucursal;
    ...
    await connection.query<ResultSetHeader>(
      ...
      [
        ...
        estado || administrativoAntiguo.estado,
        idUsuario,
      ]
    );
    ...
    await connection.query<ResultSetHeader>(
      ...
      [
        centroAsignado || null,
        sucursalAsignada || null,
        typeof id_departamento === "number"
          ? id_departamento
          : administrativoAntiguo.id_departamento,
        jornada || null,
        extension_telefonica || null,
        estado_administrativo || null,
        cargo ?? administrativoAntiguo.cargo,
        nivel_acceso ?? administrativoAntiguo.nivel_acceso,
        numero_empleado ?? administrativoAntiguo.numero_empleado,
        descripcion ?? administrativoAntiguo.descripcion,
        idAdministrativo,
      ]
    );
    ...
    // ========== RECUPERAR ADMINISTRATIVO ACTUALIZADO ==========
    const [administrativosActualizados] =
      await connection.query<AdministrativoDetalle[]>(
        `SELECT
          adm.id_administrativo,
          adm.id_usuario,
          adm.id_centro,
    ...
    const administrativoActualizado = limpiarDatosSensibles(
      administrativosActualizados[0]
    );
    ...
    await registrarLog({
      ...
      tipo: "audit",
      modulo: "administrativos",
      accion: "editar_administrativo",
      descripcion: `Administrativo editado: ${nombre} ${apellido_paterno} (ID administrativo: ${idAdministrativo}, usuario ID: ${idUsuario})`,
      objeto_tipo: "administrativo",
      objeto_id: idAdministrativo.toString(),
      datos_antiguos: limpiarDatosSensibles(administrativoAntiguo),
      datos_nuevos: body,
      ip_origen: obtenerIP(request),
      agente_usuario: obtenerUserAgent(request),
      nivel_severidad: 5,
    ...
    return NextResponse.json({
      ...
      data: administrativoActualizado,
    });
  } catch (error: any) {
    if (connection) {
      await connection.rollback();
    ...
    await registrarLog({
      ...
      modulo: "administrativos",
      accion: "editar_administrativo",
      descripcion: `Error al actualizar administrativo ID ${params.id}: ${error.message}`,
      nivel_severidad: 8,
      ip_origen: obtenerIP(request),
      agente_usuario: obtenerUserAgent(request),
    });
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Error al actualizar administrativo",
        detalles: error.message,
      },
      { status: 500 }
    );
...
// ============================================================================
// POST - ACCIONES DE SEGURIDAD / CUENTA SOBRE EL ADMINISTRATIVO (USUARIO ASOCIADO)
// ============================================================================
...
export async function POST(
  ...
  try {
    ...
    // Obtener administrativo + usuario actual
    const [rowsAdministrativo] = await connection.query<AdministrativoDetalle[]>(
      `SELECT adm.*, u.*
       FROM administrativos adm
       INNER JOIN usuarios u ON adm.id_usuario = u.id_usuario
       WHERE adm.id_administrativo = ?`,
    ...
    if (rowsAdministrativo.length === 0) {
      await connection.rollback();
      connection.release();
      return NextResponse.json(
        { success: false, error: "Administrativo no encontrado" },
    ...
    const administrativoActual = rowsAdministrativo[0];
    const idUsuarioObjetivo = administrativoActual.id_usuario;
    ...
    switch (cuerpo.action) {
      ...
      case "toggle_2fa": {
        ...
        if (enable) {
          ...
          logDescripcion = `2FA activado para usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo})`;
          ...
          logDescripcion = `2FA desactivado para usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo})`;
          nivel_severidad_log = 7;
        }
      ...
      case "reset_password": {
        ...
        logDescripcion = `Contraseña reseteada por administrador para usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo}) (sin exponer pwd en log)`;
      ...
      case "force_password_change": {
        ...
        logDescripcion = `Forzado cambio de contraseña al usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo})`;
      ...
      case "send_reset_link": {
        ...
        logDescripcion = `Token de reseteo de contraseña generado para usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo})`;
        nivel_severidad_log = 7;
        break;
      }

      // --------------------------------------------------
      // BLOQUEAR USUARIO + MARCAR ADMINISTRATIVO SUSPENDIDO
      // --------------------------------------------------
      case "block_user": {
        ...
        // actualizar estado de administrativo
        await connection.query<ResultSetHeader>(
        ...
        respuestaAccion = {
          ...
          estado_administrativo: "suspendido",
          suspension_activa: true,
        };

        logAccion = "bloquear_usuario";
        logDescripcion = `Usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo}) bloqueado. Motivo: ${motivo}`;
        nivel_severidad_log = 9;
        break;
      }

      // --------------------------------------------------
      // DESBLOQUEAR USUARIO + REACTIVAR ADMINISTRATIVO
      // --------------------------------------------------
      case "unblock_user": {
        await connection.query<ResultSetHeader>(
          `UPDATE usuarios
        ...
        logDescripcion = `Usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo}) desbloqueado por admin`;
      ...
      case "reset_failed_attempts": {
        ...
        logDescripcion = `Intentos fallidos reseteados para usuario ID ${idUsuarioObjetivo} (administrativo ID ${idAdministrativo})`;
    ...
    await registrarLog({
      ...
      objeto_tipo: "administrativo",
      objeto_id: idAdministrativo.toString(),
      datos_antiguos: limpiarDatosSensibles(administrativoActual),
    ...
    console.error("❌ Error en acción de seguridad administrativo:", error);

    await registrarLog({
      id_usuario: null,
      tipo: "error",
      modulo: "administrativos",
      accion: "accion_seguridad_administrativo",
      descripcion: `Error al ejecutar acción de seguridad en administrativo ID ${params.id}: ${error.message}`,
      nivel_severidad: 9,
      ip_origen: obtenerIP(request),
      agente_usuario: obtenerUserAgent(request),
    });
...
// ============================================================================
// DELETE - ELIMINAR ADMINISTRATIVO (ELIMINA USUARIO SI NO TIENE DEPENDENCIAS)
// ============================================================================
...
export async function DELETE(
  ...
  try {
    ...
    // 1. ¿Existe el administrativo + usuario?
    const [rowsAdministrativo] = await connection.query<AdministrativoDetalle[]>(
      `SELECT adm.*, u.*
       FROM administrativos adm
       INNER JOIN usuarios u ON adm.id_usuario = u.id_usuario
    ...
    const administrativo = rowsAdministrativo[0];
    const idUsuario = administrativo.id_usuario;
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "No se puede eliminar el administrativo porque el usuario asociado tiene registros históricos",
          detalles: dependenciasDetalle,
          sugerencia:
            "Cambiar el estado del usuario/administrativo a 'inactivo' o 'bloqueado' en lugar de eliminarlo para mantener la integridad de los datos.",
        },
        { status: 400 }
      );
    }
    ...
    // NOTA:
    // - administrativos tiene ON DELETE CASCADE con usuarios
    // - medicos / tecnicos también
    // - pacientes.id_usuario suele ser ON DELETE SET NULL

    // 5. Eliminar usuario (administrativo cae en cascada)
    await connection.query<ResultSetHeader>(
    ...
    await registrarLog({
      ...
      tipo: "security",
      modulo: "administrativos",
      accion: "eliminar_administrativo",
      descripcion: `Administrativo eliminado: ${administrativo.nombre} ${administrativo.apellido_paterno} (ID administrativo: ${idAdministrativo}, usuario ID: ${idUsuario}, Username: ${administrativo.username}, RUT: ${administrativo.rut})`,
      objeto_tipo: "administrativo",
      objeto_id: idAdministrativo.toString(),
      datos_antiguos: limpiarDatosSensibles(administrativo),
      ip_origen: obtenerIP(request),
      agente_usuario: obtenerUserAgent(request),
      nivel_severidad: 9,
    });
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        nombre_completo: `${administrativo.nombre} ${administrativo.apellido_paterno}${
          administrativo.apellido_materno ? " " + administrativo.apellido_materno : ""
        }`,
        username: administrativo.username,
        rut: administrativo.rut,
      },
    });
  } catch (error: any) {
    if (connection) {
    ...
    await registrarLog({
      ...
      modulo: "administrativos",
      accion: "eliminar_administrativo",
      descripcion: `Error al eliminar administrativo ID ${params.id}: ${error.message}`,
      nivel_severidad: 9,
      ip_origen: obtenerIP(request),
      agente_usuario: obtenerUserAgent(request),
    });
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Error al eliminar administrativo",
        detalles: error.message,
      },
      { status: 500 }
    );
