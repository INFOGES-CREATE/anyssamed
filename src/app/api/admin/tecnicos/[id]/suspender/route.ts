...
export async function POST(
  ...
  try {
    ...
    // Cerrar sesiones activas (CORREGIDO)
    await connection.query(
      `UPDATE sesiones_usuario 
       SET estado = 'forzada',
           motivo_cierre = 'Usuario suspendido',
           fecha_cierre = NOW(),
           cerrado_por = ?
    ...
    // Cancelar citas futuras (CORREGIDO)
    await connection.query(
      `UPDATE citas 
       SET estado = 'cancelada',
           notas = CONCAT('[CANCELADA POR SUSPENSIÓN] ', notas)
