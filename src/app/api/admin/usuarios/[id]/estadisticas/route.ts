  ...
  try {
    ...
    const [estadisticasGenerales] = await connection.query<RowDataPacket[]>(
      `SELECT 
        ...
        COALESCE((SELECT COUNT(*) FROM citas WHERE (id_paciente = ? OR id_profesional = ?) AND estado = 'programada'), 0) as citas_programadas,
        COALESCE((SELECT COUNT(*) FROM citas WHERE (id_paciente = ? OR id_profesional = ?) AND estado = 'confirmada'), 0) as citas_confirmadas,
        COALESCE((SELECT COUNT(*) FROM citas WHERE (id_paciente = ? OR id_profesional = ?) AND estado = 'no_asistio'), 0) as citas_no_asistio,
        COALESCE((SELECT COUNT(*) FROM citas WHERE (id_paciente = ? OR id_profesional = ?) AND fecha_hora_inicio >= CURDATE()), 0) as citas_futuras,
        COALESCE((SELECT COUNT(*) FROM citas WHERE (id_paciente = ? OR id_profesional = ?) AND prioridad = 'urgente'), 0) as citas_urgentes,
        COALESCE((SELECT SUM(monto) FROM citas WHERE (id_paciente = ? OR id_profesional = ?) AND pagada = 1), 0) as ingresos_totales,
        COALESCE((SELECT COUNT(*) FROM citas WHERE (id_paciente = ? OR id_profesional = ?) AND pagada = 0), 0) as citas_sin_pagar,
        ...
        (SELECT COUNT(DISTINCT id_paciente) FROM citas WHERE id_profesional = ?) as pacientes_atendidos,
        (SELECT COUNT(DISTINCT id_profesional) FROM citas WHERE id_paciente = ?) as medicos_consultados
      `,
      [
    ...
    const [citasPorEstado] = await connection.query<RowDataPacket[]>(
      `SELECT 
        estado,
        COUNT(*) as cantidad,
        ROUND((COUNT(*) * 100.0 / (SELECT COUNT(*) FROM citas WHERE id_paciente = ? OR id_profesional = ?)), 2) as porcentaje
      FROM citas
    ...
    const [citasRecientes] = await connection.query<RowDataPacket[]>(
      `SELECT 
        ...
        CASE 
          WHEN c.id_profesional = ? THEN CONCAT(u.nombre, ' ', u.apellido_paterno)
          ELSE 'Médico'
        END as nombre_medico
      FROM citas c
      LEFT JOIN usuarios u ON (c.id_paciente = u.id_usuario OR c.id_profesional = u.id_usuario)
      WHERE c.id_paciente = ? OR c.id_profesional = ?
