// app/api/medico/agenda/citas/[id]/iniciar/route.ts
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [citas] = await pool.query<RowDataPacket[]>(
      `SELECT id_cita, estado, id_paciente FROM citas WHERE id_cita = ? AND id_medico = ?`,
      [idCita, idMedico]
    );
    ...
    await pool.query(
      `INSERT INTO historial_cambios_citas 
       ...
       VALUES (?, ?, 'estado', ?, 'en_atencion', 'inicio_atencion')`,
