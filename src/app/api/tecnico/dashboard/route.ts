...
async function obtenerEstadisticas(
  ...
  try {
    ...
    const queries = [
      ...
      // Tareas pendientes
      pool.query<RowDataPacket[]>(
        `SELECT COUNT(*) as total FROM tareas_secretaria 
         WHERE id_secretaria = ? AND estado = 'pendiente'`,
        [idSecretaria]
      ),
      // Mensajes sin leer
      pool.query<RowDataPacket[]>(
...
/**
 * Obtiene las tareas pendientes de la secretaria
 */
  ...
  try {
    const [rows] = await pool.query<RowDataPacket[]>(
      ...
      SELECT 
        ...
        ts.estado,
        CONCAT(u.nombre, ' ', u.apellido_paterno) as asignado_a,
        ts.fecha_creacion,
        ts.url_accion
      FROM tareas_secretaria ts
      INNER JOIN usuarios u ON ts.id_usuario_asignado = u.id_usuario
      WHERE ts.id_secretaria = ?
        AND ts.estado IN ('pendiente', 'en_progreso')
      ORDER BY 
        CASE 
          WHEN ts.prioridad = 'alta' THEN 1
          WHEN ts.prioridad = 'media' THEN 2
          ELSE 3
        END ASC,
        ts.fecha_vencimiento ASC
      LIMIT 15
      `,
      [idSecretaria]
    );
  ...
  try {
    ...
    const [tareasVencidas] = await pool.query<RowDataPacket[]>(
      ...
      WHERE ts.id_secretaria = ?
        ...
        AND ts.fecha_vencimiento < CURDATE()
      ORDER BY ts.fecha_vencimiento ASC
      LIMIT 5
      `,
      [idSecretaria]
    );
    ...
    for (const tarea of tareasVencidas) {
      alertas.push({
        id_alerta: idAlerta++,
        tipo: tarea.prioridad === "alta" ? "alta" : "media",
